...
| **Medium** | RXAS Import Stub Fidelity | `rximpf_f()` still rebuilds imported declarations by parsing synthetic Rexx source from RXAS metadata. It now preserves `expose`, scalar optional arguments and task-method kind/Level G gating well enough for ADDRESS and concurrency imports, but richer metadata or a direct AST importer is needed so original defaults and complex typed optionals are not approximated. | Partially Fixed |
