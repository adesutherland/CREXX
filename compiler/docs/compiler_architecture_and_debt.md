...
| `rxcp_val_*.c` | Validation pipeline (check, sym, type, trans, orch). See [Exit Bridge Guide](exit_bridge_guide.md). |
...
| **High** | Optimizer Flow Analysis Roadmap | Assess both RXAS and `rxc` flow-analysis capabilities for driving safe optimisations. RXAS should grow a per-procedure control-flow graph from labels plus the instruction database `FLOW_NEXT` / `FLOW_COND` / `FLOW_JUMP` / barrier metadata, extend the instruction database with def/use/kill metadata, then apply classic dataflow analyses such as liveness, reaching definitions, dead temporary elimination, branch threading, and safe superinstruction selection. Compiler-emitted `.srcstep` / `.traceevent` metadata gives RXAS useful register/source-value intent hints, but correctness must not depend on that metadata because optimized-away values and hand-written RXAS may not carry complete trace records. Near-term keyhole rules are local and conservative; broader superinstructions, compare/branch fusion for arbitrary hand-written RXAS, and call/argument cleanup need liveness and control-flow proof instead of pattern-only assumptions. | Roadmap |
| **High** | Register Allocation Lifetime Reuse | Revisit fixed variable-to-register assignment. The current stable assignment model made later compiler stages simpler, but hot procedures can keep larger frames than necessary after variables go out of scope or after inlining. Analyse whether register slots can be safely relaxed/reused once variable lifetime ends, especially for compiler-generated temporaries and inlined helpers. | Roadmap |
| **Medium** | RXAS Keyhole Liveness Proof | Add liveness checks around keyhole rules that delete producer instructions, including compare-to-branch fusion. The current rule engine proves hazards between matched instructions, but does not prove that a deleted temporary result is dead on all fallthrough paths; future rules should use local or CFG-backed liveness before expanding beyond compiler-shaped patterns. | Roadmap |
| **Medium** | Plugin Interface | Implement "Analyze-Consult-Negotiate" loop in `rxcp_val_orch.c`. | **RESOLVED** |
| **Medium** | RXAS Import Stub Fidelity | `rximpf_f()` still rebuilds imported declarations by parsing synthetic Rexx source from RXAS metadata. It now preserves `expose`, scalar optional arguments and Gate F task-method kind/Level G gating well enough for ADDRESS and concurrency imports, but richer metadata or a direct AST importer is needed so original defaults and complex typed optionals are not approximated. | Partially Fixed |
| **Medium** | Interface Object Reference Semantics | ADDRESS sandbox/stem tracing exposed two separate risks. The VM-side `rxvml` nested-call state is now preserved, ADDRESS requests now own per-command sandbox/stem binding objects during dispatch, standard sandbox/stem C writes use direct VM-layout helpers, and Rexx/C providers have stable request-level read/write helpers. Broader compiler/runtime aliasing tests are still needed for interface-typed helper returns and temporary variables before more object-heavy provider abstractions depend on them. | Partially Fixed |
| **Medium** | Default Sandbox Lifetime | Resolved by contract: ADDRESS retains the current/default environment, but sandbox objects are caller-owned Rexx capabilities and must be passed on each command that needs them. No retained default sandbox object is kept in ADDRESS state. | **RESOLVED** |
