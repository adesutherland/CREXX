...
def main():
    ...
    def run(label, argv, marker=None, expected=0, env=None, cwd=work):
        argv = list(map(str, argv))
        path = logs / (label + '.log')
        with path.open('w', encoding='utf-8') as log:
            log.write('argv=' + repr(argv) + '\n'); log.flush()
            result = subprocess.run(argv, cwd=cwd, env=env or clean, stdout=log,
    ...
    try:
        ...
        run('engine', [copied_helper, fixture], 'PASS: release engine smoke', cwd=engine)
        ...
        run('altered-backend-rejected', [copied_helper, fixture],
            'no usable verified CPU backend variant', expected=1, cwd=engine)
        (engine / 'rxllama.runtime.json').unlink()
        run('missing-manifest-rejected', [copied_helper, fixture], expected=1, cwd=engine)
        ...
        for mode in ('opt', 'noopt'):
            ...
            run(mode + '-compile', [prefix / ('bin/rxc' + suffix), '--no-exe-import',
                *(['-n'] if mode == 'noopt' else []), '-i', prefix / 'bin', '-o', program, src])
            run(mode + '-assemble', [prefix / ('bin/rxas' + suffix), '-o', program, program])
        ...
        run('relocated-native', [relocated / executable.name, fixture, fixture_info['sha256']], marker, cwd=relocated)
        outcome = 'passed'
    finally:
