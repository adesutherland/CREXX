...
cases = (
    ("typed_configuration", source / "tests/native-inference/typed_configuration.crexx", "PASS: typed llama configuration"),
    ("typed_embeddings", source / "tests/native-inference/typed_embeddings.crexx", "PASS: typed llama embeddings"),
    ("persistent_embeddings", prefix / "share/crexx/llama/examples/persistent_embeddings.crexx", "PASS: persistent typed embedding example"),
    ("shared_embeddings", prefix / "share/crexx/llama/examples/shared_embeddings.crexx", "PASS: four typed embedding workers"),
)
for opt in ("opt", "noopt"):
  for name, original_source, marker in cases:
    test = opt + "-" + name
    test_modes = modes if name != "typed_configuration" else ["cpu"]
...
print("PASS: typed STEP-04 installed and relocated native consumers, opt/noopt")
