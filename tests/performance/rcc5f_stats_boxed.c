...
PROCEDURE(mean)
{
    ...
    if (NUM_ARGS != 1)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.MEAN expects one array")
    rxstats_accumulate(ARG0, &moments);
    if (moments.count < 1)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.MEAN requires at least one value")
...
PROCEDURE(stddev)
{
    rxstats_moments moments;
    if (NUM_ARGS != 1)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.STDDEV expects one array")
    rxstats_accumulate(ARG0, &moments);
    if (moments.count < 2)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.STDDEV requires at least two values")
...
PROCEDURE(covariance)
{
    ...
    if (NUM_ARGS != 2)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.COVARIANCE expects two arrays")
    if (!rxstats_accumulate_pairs(ARG0, ARG1, &moments))
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.COVARIANCE requires equal-length arrays")
    if (moments.count < 2)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.COVARIANCE requires at least two pairs")
...
PROCEDURE(correlation)
{
    rxstats_pair_moments moments;
    if (NUM_ARGS != 2)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.CORRELATION expects two arrays")
    if (!rxstats_accumulate_pairs(ARG0, ARG1, &moments))
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.CORRELATION requires equal-length arrays")
    if (moments.count < 2)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.CORRELATION requires at least two pairs")
    if (moments.m2_x == 0.0 || moments.m2_y == 0.0)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.CORRELATION is undefined for zero variance")
...
PROCEDURE(regression)
{
    ...
    if (NUM_ARGS != 4)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.REGRESSION expects two arrays, slope, and intercept")
    if (!rxstats_accumulate_pairs(ARG0, ARG1, &moments))
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.REGRESSION requires equal-length arrays")
    if (moments.count < 2)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.REGRESSION requires at least two pairs")
    if (moments.m2_x == 0.0)
        RETURNSIGNAL(SIGNAL_INVALID_ARGUMENTS, "RCC5FBOXED.REGRESSION is undefined for zero x variance")
...
LOADFUNCS
    ADDPROC(mean, "rcc5fboxed.mean", "b", ".float", "expose values = .float[]");
    ADDPROC(stddev, "rcc5fboxed.stddev", "b", ".float", "expose values = .float[]");
    ADDPROC(covariance, "rcc5fboxed.covariance", "b", ".float", "expose x = .float[],expose y = .float[]");
    ADDPROC(correlation, "rcc5fboxed.correlation", "b", ".float", "expose x = .float[],expose y = .float[]");
    ADDPROC(regression, "rcc5fboxed.regression", "b", ".float", "expose x = .float[],expose y = .float[],expose slope = .float,expose intercept = .float");
ENDLOADFUNCS
