...
//     strSubproc strProc;
   ...
   //     current_frame->locals[(pc + (3))->index];
     ...
     * reg 3 is an array of
     *       - 3 opaque REDIRECT binary structures (e.g. generated by redrtoarr)
     *         reg3[1] = input, reg3[2] = output, reg3[3] = error
     *       - reg3[4] holds the environment variables.
     *         reg3[4] is an array of names and arguments interlaced as setup
     ...
     *       - reg3[5] is flags stored in the integer attribute of the register, values are
     *           0 - Use native environment variables
     *           1 - Use crexxsaa http server protocol
     *
     *       This instruction just needs to pass all these to the shellspawn() function
    ...
    START_INSTRUCTION(SPAWN_REG_REG_REG) CALC_DISPATCH(3)
            DEBUG("TRACE - SPAWN R%lu,R%lu,R%lu\n", REG_IDX(1), REG_IDX(2), REG_IDX(3));
            {
                ...
                /* op3R->attributes[3] are the environment variables */
                /* op3R->attributes[4] are the flags */
                if (shellspawn(command, pIn, pOut, pErr, op3R->attributes[3], op3R->attributes[4]->int_value, &command_rc, &errorText)) {
...
#include "instrmiss.h"
    ...
    SIGNAL:
        if (signal_details) {
            fprintf(stderr,"\nSignal %s (%s) Received - No Signal Handler so Aborting\n", signal_type, signal_details);
            free(signal_details);
        }
