...
call lineout out_file, "static char* canonical_composer(const char *cp1, const char *cp2, int *out_len) {"
...
call lineout out_file, "    *byte = cp1[0];"
...
call lineout out_file, "    *byte = cp2[0];"
call lineout out_file, "    if (*byte < 0x80) char_len = 1;"
call lineout out_file, "    else if (*byte < 0xE0) char_len = 2;"
call lineout out_file, "    else if (*byte < 0xF0) char_len = 3;"
call lineout out_file, "    else char_len = 4;"
...
do c = 1 to canonical_compos.0
    ...
    find = find || '"'
    call lineout out_file, '   ' find '{ *out_len =' utf8_bytes(code_point)'; return' to_utf8(code_point)';} // -> U+'code_point
end
call lineout out_file, ""
...
call lineout out_file, "    const char* single_cp;"
call lineout out_file, "    int single_cp_len;"
call lineout out_file, "    int single_cp_last_starter;"
call lineout out_file, "    int single_ccc;"
call lineout out_file, "    int i, j, k;"
...
/*
    These are the treatment classifications / goto targets (IS=Inert Starter S=Starter IX=Inert Single X=Single)
       ...
       TP2. S -> starter
       TP3. IX (IX)* -> inert_single
       TP4. X -> full
       TP5. IS (X|IX)+ IS -> inert_starter
       TP6. S (X|IX)+ IS -> semi_inert_starter
       TP7. S (X|IX)+ S -> starter
       TP8. IS (IX)+ inert_starter_then_non_starter
       TP9. (S|IS) (X|IX)+ starter_then_non_starter
       TP10. Otherwise -> full
*/
...
if inert_ccc_class.0 \= "" then do
    call lineout out_file, '        inert_ccc_class_'0 '{single_cp = (char*)start; single_cp_len = (int)(str - start); goto inert_starter;} // TP1'
end
do cc = 1 to 255 /* Currently the highest canonical_combining_class is 254, 255 for future proofing a bit */
    if inert_ccc_class.cc \= "" then do
        call lineout out_file, '        inert_ccc_class_'cc '{single_ccc =' cc'; single_cp = (char*)start; single_cp_len = (int)(str - start); goto inert_single;} // TP3'
    end
end
call lineout out_file, ""
...
if non_inert_ccc_class.0 \= "" then do
    call lineout out_file, '        non_inert_ccc_class_'0 '{single_cp = (char*)start; single_cp_len = (int)(str - start); single_cp_last_starter = 0; goto starter;} // TP2'
end
do cc = 1 to 255 /* Currently the highest canonical_combining_class is 254, 255 for future proofing a bit */
    if non_inert_ccc_class.cc \= "" then do
        call lineout out_file, '        non_inert_ccc_class_'cc '{single_ccc =' cc'; single_cp = (char*)start; single_cp_len = (int)(str - start); goto full;} // TP4'
    end
end
call lineout out_file, ""
        ...
        if num_code_points = 1 then do
            ...
            */
            ...
            select
                ...
                when ccc = 0 then do
                    call lineout out_file, '        "'escape(code_point,length(strip(code_point,"L","0")))'" {single_cp = (char*)'to_utf8(canonical_composed)'; single_cp_len =' utf8_bytes(canonical_composed)'; single_cp_last_starter = 0; goto starter;} // TP2' code_point'->'canonical_composed
                ...
                otherwise do /* ccc > 0 and inert = 0 */
                    call lineout out_file, '        "'escape(code_point,length(strip(code_point,"L","0")))'" {single_ccc =' ccc'; single_cp = (char*)'to_utf8(canonical_composed)'; single_cp_len =' utf8_bytes(canonical_composed)'; goto full;} // TP4' code_point'->'canonical_composed
                    type_count._CAN_RECOM_SINGLE = type_count._CAN_RECOM_SINGLE + 1
                end
            end
        end
            ...
            if canonical_combining_class.cd = 0 & canonical_combining_class.last_cd = 0 then do
                /* Multiple codepoints start and ending with ccc = 0
                   ...
                   6. S (X|IX)+ IS -> semi_inert_starter
                   7. S (X|IX)+ S -> starter
                */
                select
                    when inert.cd & inert.last_cd then do
                        /* 5. IS (X|IX)+ IS -> inert_starter */
                    ...
                    when inert.last_cd then do
                        /* 6. S (X|IX)+ IS -> semi_inert_starter */
                        call lineout out_file, '        "'escape(code_point,length(strip(code_point,"L","0")))'" {single_cp = (char*)'to_utf8_string(canonical_composed)'; single_cp_len =' utf8_string_bytes(canonical_composed)'; goto semi_inert_starter;} // TP6' code_point'->'canonical_composed
                        type_count._CAN_RECOM_SEMI_INERT_STARTER = type_count._CAN_RECOM_SEMI_INERT_STARTER + 1
                    end
                    otherwise do
                        total_length = utf8_string_bytes(canonical_composed)
                        last_starter = total_length - utf8_string_bytes(last_cd)
                        call lineout out_file, '        "'escape(code_point,length(strip(code_point,"L","0")))'" {single_cp = (char*)'to_utf8_string(canonical_composed)'; single_cp_len =' total_length'; single_cp_last_starter =' last_starter'; goto starter;} // TP7' code_point'->'canonical_composed
                        type_count._CAN_RECOM_STARTER = type_count._CAN_RECOM_STARTER + 1
                    end
                end
            end
                ...
                /*
                       ...
                       9. (S|IS) (X|IX)+ starter_then_non_starter
                */
                starter_followed_by_non_starters = 1
                last_starter = 0
                if canonical_combining_class.cd = 0 then do
                ...
                if starter_followed_by_non_starters then do
                    ...
                    if inert then do
                        ...
                        call lineout out_file, '        "'escape(code_point,length(strip(code_point,"L","0")))'" {single_cp = (char*)'to_utf8_string(subword(canonical_composed,1,last_starter)) ||,
                            '; single_cp_len =' utf8_string_bytes(subword(canonical_composed,1,last_starter)) ||,
                            ';' mult 'goto starter_then_non_starter;} // TP9' code_point'->'canonical_composed
                        type_count._CAN_RECOM_STARTER_THEN_NON_STARTER = type_count._CAN_RECOM_STARTER_THEN_NON_STARTER + 1
                    end
                end
                    ...
                    */
                    ...
                    /* Branch */
                    if inert then do
                        call lineout out_file, '        "'escape(code_point,length(strip(code_point,"L","0")))'" {' mult 'goto inert_single;} // TP3' code_point'->'canonical_composed
                        type_count._CAN_RECOM_INERT_SINGLE = type_count._CAN_RECOM_INERT_SINGLE + 1
                    end
                    else do
                        call lineout out_file, '        "'escape(code_point,length(strip(code_point,"L","0")))'" {' mult 'goto full;} // TP10' code_point'->'canonical_composed
...
say "Canonical TP6  Recomposition Semi-Inert Starter" type_count._CAN_RECOM_SEMI_INERT_STARTER
say "Canonical TP7  Recomposition Non-Inert Starter" type_count._CAN_RECOM_STARTER
...
say "Canonical TP10 Recomposition Non-Inert Complex" type_count._CAN_RECOM_COMPLEX
...
write_nfkc_and_nfc_normaliser_tail: procedure expose out_file
    ...
    call lineout out_file, "            for (i = 0; i < single_cp_len - 1; i++) {"
    call lineout out_file, "               out[(*out_len)++] = single_cp[i];"
    call lineout out_file, "            }"
