...
| Linux / Windows / CUDA / Vulkan | Hosted package qualification is active on isolated candidate branches. Linux CPU/Vulkan and macOS arm64 CPU/Metal fixture packages pass at `2bc56249d`; Intel Mac also passes its isolated replay at `e39916f2a`, with the original stall still unexplained. Windows portability retries and CUDA builds are pending. Real BGE/Smol device cells remain open. See the [pipeline ledger](../native-inference-ci/README.md). |
