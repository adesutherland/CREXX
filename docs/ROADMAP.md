...
| Exact packed vector CPU provider | [`RXVECTOR-01`](../performance/RXVECTOR-01-WORKLIST.md) | **Accepted and complete on macOS.** The process-reentrant Level G `rxvector` provider exposes explicit portable `f32le` conversion and exact deterministic cosine/top-k over BINARY-01 owners. The accepted profiling-off Release verdict reaches the shared direct-kernel ceiling and 14.68x-15.62x paired speedup over the Level B oracle. Installed dynamic/static selection, public documentation, bounded `crexx-rag` integration, and proportional normal-Debug QA are complete; supported Linux sanitizer/cross-platform qualification remains release QA. |
