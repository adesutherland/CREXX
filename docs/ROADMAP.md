...
| Theme | Source discussions | Direction |
|-------|--------------------|-----------|
| Runtime capability composition | [`planning/release-1/runtime-capability-composition-roadmap.md`](planning/release-1/runtime-capability-composition-roadmap.md), PERF3-07 `CAP-05`, CREXXRAG-SHA256 | RCC-1 through RCC-4 are implemented in the current tree: declarative native-provider dependencies, static-first trusted dynamic resolution, automatic native-package selection, explicit RXBIN module initialization, and the B+G standard/default `rx_hash` provider with `rxhash.sha256()`. The opcode disposition list and structural format boundary remain accepted: existing metadata homes stay 007; a file-structure change moves to 008. Historical `system`/`rxmath` partitioning and compatibility-gated RXAS instruction-to-call work remain later gates. |
