...
| UNICODE-NORM-02-F1: exact scalar accessor through a class-attribute receiver copies the whole object | [`UNICODE-NORM-02-F1` performance activity](../performance/ROADMAP.md#activity-register) | **Open RXC inliner performance defect; not a correctness defect in the accessor.** The exact read-only scalar-getter proof recognizes the getter, but class attributes are excluded from receiver sharing, so the inlined body detaches the receiver with a whole-value `COPY_REG_REG` on every activation. The Unicode NFD `status()` diagnostic exposes a material cost while preserving the same result. Retain the ordinary API check, promote the scratch measurements into a governed evidence bundle, and prove a bounded scoped receiver-storage alias with initialization, storage-identity, signal-cleanup, TRACE, optimized/no-opt and dual-VM guards before any production selection. The performance-roadmap entry is the execution and evidence authority; this cross-reference records the defect and does not approve implementation. |
| CRI-18: branch-local typed-object value is not merged across an `if`/`else` join | Downstream `crexx-rag` Codex extraction plus an independent 28-line Level-G reproducer; reconfirmed on `origin/develop` at `1548ba749` | **Open CREXX compiler defect; the previous 255-register diagnosis was disproved.** When the same typed variable receives its first assignment independently in both branches, `rxc` allocates distinct branch-local registers. The true arm copies its object to one register, emits `endlife` for that register and branches to the join; the false arm copies its object to a different register, which is the only one used after the join. Taking the true arm therefore accesses the wrong object and raises `SIGNAL OUT_OF_RANGE`. The minimal reproducer fails with optimized and non-optimized compilation on both `rxbvm` and `rxtvm`; initializing the typed object once in the enclosing scope makes all four cells pass. The original unsplit `crexx-rag` image and the restored current application reproduce the same generated-instruction fault, while enclosing-scope initialization allows the unsplit application and its crash-recovery regression to pass. Fix the compiler's lexical-scope/control-flow value merge so every predecessor supplies the joined value, retain the minimal opt/no-opt dual-VM regression, and verify linked/native packaging. The source-level enclosing-scope initialization is containment, not the CREXX fix. |
