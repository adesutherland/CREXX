...
| CRI-18: disjoint block-local and enclosing values did not warn in a mixed `if`/`else` | Downstream `crexx-rag` Codex extraction plus an independent 28-line Level-G reproducer; re-audited on `origin/develop` at `273c7b46` | **Reclassified as a source defect plus missing diagnostic coverage; there is no value-merge or register-allocation defect.** A grouped `DO` arm creates a local scope, while a single-statement arm executes in the enclosing scope. The two `result` assignments therefore create different variables, and the later read legally selects the enclosing variable; taking the grouped arm leaves that object invalid and the expected method access raises `SIGNAL OUT_OF_RANGE`. A lone later read already produced `#NOT_IN_SAME_SCOPE`, but the validator suppressed the warning when the new enclosing binding was created by an implicit assignment, so the exact reproducer compiled silently. The hotfix diagnostic includes implicit assignments in the disjoint-scope warning while retaining legal execution: an explicit declaration suppresses the warning and a previously visible parent is reused quietly. The user documentation now restores lexical scope and removes the stale semantic-hoisting description left by the March 2026 experiment and rollback. Downstream generated code must establish the enclosing result before the branches when it intends one joined value. The focused diagnostic contract covers the original mixed shape, implicit reassignment, taken-constant use, explicit intent and parent reuse. The compiler-labelled Debug sweep passes 227/227, and combined full-Debug evidence covers 2,386/2,386 tests after making four intentional loop-local reuses explicit and rerunning the sandbox-blocked socket, process and uptime tests with host permissions. The all-target build also exposes substantial existing first-party implicit-name warning debt for separate audited cleanup; that debt is not evidence of CRI-18 runtime miscompilation. |
