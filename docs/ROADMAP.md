...
| Performance benchmark portfolio expansion | [`performance closeout plan`](../performance/PERFORMANCE-CLOSEOUT-PLAN.md) | POSTPERF-05 is complete and no further production performance stage is authorized. Portfolio v3 retains all 14 AWFY lanes plus Base64, RexxCPS and separate capability controls. Stage 4 completed the final fair cREXX source review, honest ooRexx/NetRexx capability classification and calibration; Stage 5 produces the current pre-release scorecard. Direct Java and CPython remain separate controls, and primitive-Java timed work is not reported as NetRexx capability. Any test revision prompted by an honest adverse result must preserve algorithm, work and observable result and repeat the affected qualification. |
