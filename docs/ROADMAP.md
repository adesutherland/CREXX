...
| UNICODE-NORM-02-F1: exact scalar accessor through a class-attribute receiver copies the whole object | [`LEVEL-L-INLINE-01` and `UNICODE-NORM-02-F1` performance activities](../performance/ROADMAP.md#activity-register) | **Bounded receiver-storage repair staged on local hotfix; focused proof and first Release verdict passed; broader QA pending.** Exact call-free read-only scalar getters can now borrow proved receiver storage without a whole-value `COPY_REG_REG`; imported methods use the same receiver gate as local methods, and statement-owned indexed mutation retains evaluate-once copyback. References, escapes, whole-value replacement, dynamic/interface dispatch and unproved receivers remain ordinary calls. The retained optimized/no-opt, RXDAS and dual-VM proof plus the 2026-08-26 profiling-off Release evidence cover the admitted shape. Publication to `develop` remains gated on hotfix QA. |
| RXC-CALL-REG-01: discarded call temporaries inflate procedure local counts | [`RXC-CALL-REG-01` performance activity](../performance/ROADMAP.md#activity-register) | **Register-lifetime repair staged on local hotfix; focused optimized/no-opt runtime proof passed; broader QA pending.** RXC now recycles discarded standalone-call results, temporary member receivers outside the reserved call window, and statement-call node registers before deferred cleanup. A permanent 32-call regression caps the dispatch procedure at 7 optimized and 39 no-opt locals; the retained pre-fix no-opt image required 101. The Unicode scale diagnostic fell from 6,455 to 361 locals in scratch, but did not materially improve normalization wall time; this is a compiler resource repair, not evidence for selecting the generated normalizer. |
