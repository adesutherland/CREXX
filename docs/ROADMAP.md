...
| Performance benchmark portfolio expansion | [`POST-PERF3-WORKLIST.md`](../performance/POST-PERF3-WORKLIST.md) | Full AWFY Json, DeltaBlue, CD and Havlak are qualified reserve lanes and change no aggregate. DeltaBlue's 4.25x optimized RXAS expansion/16.81x-18.32x bounded process gap, CD's 2.87x RXAS expansion/5.16x-5.41x bounded process gap and Havlak's 2.24x optimized RXAS expansion remain inputs to the active POSTPERF-05 bounded hoisting/register-finalisation/late-inlining stage, not hidden by benchmark rewrites. Each addition needs pinned provenance, semantic equivalence, deterministic correctness, opt/no-opt and product/control qualification before timing or promotion. Direct Java and CPython ports are separately labelled language controls, never inferred Rexx aggregate members. |
...
| Aligned typed binary storage and byte interoperability | `BINARY-01` (transferred from `POSTPERF-04-PACKED-01`) | **Future roadmap item; not in the active post-PERF3 sequence.** Decided direction: `.binary` storage bases are aligned for supported native scalar payloads; the normal typed route is native, aligned and indexed in whole elements, while a separate explicit raw/interchange route owns byte positions, fixed encoded widths such as `.f32`, endianness, conversion and boundary checks. Open decisions remain element origin, ownership of the existing `<at..type>` spelling, raw syntax and breaking-change/migration policy. When separately approved, execute it as a substantial coherent design and implementation stage rather than integer-only or read-only fragments. It does not block generic scalar access or bounded optimizer work. |
