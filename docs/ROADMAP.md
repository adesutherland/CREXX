...
| Native packed numeric storage and byte interoperability | [`BINARY-01`](../performance/BINARY-01-WORKLIST.md) (transferred from `POSTPERF-04-PACKED-01`) | **Future production gate; representation direction approved.** The fast path is only a contiguous, aligned sequence of host-native `rxinteger` or host-native `rxfloat` elements. It performs no endian or width conversion and is not an interchange format. The separate raw `.binary` route retains byte positions, encoded widths such as `i32`/`f32`, endianness, conversion, unaligned access and boundary checks. Element origin, syntax, ownership/view shape and migration remain open. `rxstats` is the intended first bulk consumer and later `rxvector` work depends on the same boundary. |
...
| Theme | Source discussions | Direction |
|-------|--------------------|-----------|
| Runtime capability composition | [`planning/release-1/runtime-capability-composition-roadmap.md`](planning/release-1/runtime-capability-composition-roadmap.md), PERF3-07 `CAP-05`, CREXXRAG-SHA256 | RCC-1 through RCC-5E are implemented in the current tree: declarative provider dependencies, static-first trusted dynamic resolution, automatic native-package selection, explicit per-mutable-module-instance initialization, `rx_hash`, the `rxfloat`/`rxint`/`rxdecimal` family, transitional boxed `rxstats`, and the `rxid`/`rxfs`/`rxplatform` split. The broad pre-release `system` and mixed `rxmath` providers are retired; only direct scalar `rxmath` compatibility names remain in `rxfloat`. Existing metadata homes stay RXBIN 007; a file-structure change moves to 008. `BINARY-01` and RCC-5F own the packed statistics contract, while compatibility-gated RXAS instruction-to-call work remains later. |
