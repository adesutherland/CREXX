...
Status: milestone 1 implemented; profiling and representation selection next
...
Milestone 1 implements the portable 007 container, sealed semantic graph,
...
validation. The append-only late-load overlay, policy-view schemas, and
instruction-site caches remain T6 follow-on work: measurements in section 7
will select their physical representation.
...
not remove the semantic graph required by execution. Compression remains a
future per-section choice. Milestone 1 stores every section uncompressed and
materializes a checked in-memory graph and native execution pool. Borrowed or
zero-copy views require measurement and a later implementation change; they
are not claimed by the current reader.
...
directory, one another, or the end of file. Compression is not selected for
the base migration: `stored_size == expanded_size` and section compression
flags are zero. A later measured change may add a versioned compression flag.
...
and checks counts and arithmetic before allocating derived tables. It rejects
...
files; and RXDAS graph diagnostic text. Additional fuzz/property coverage for
...
graph and process-local procedure bindings. Runtime relationship decisions use
the graph indexes and numeric operands; canonical metadata remains available
for generic introspection and compatibility fallbacks.

Object values carry the graph pointer and runtime type ID, with canonical text
retained for cross-image/native compatibility. Numeric type/member/factory
operands provide the sealed-image fast path. Generation-guarded monomorphic,
small-polymorphic, and megamorphic site caches remain a profiling-selected T6
follow-on and will not mutate serialized bytecode.
...
overlay built by the same graph library. Milestone 1 preserves the existing
late/native compatibility path using canonical text when graph identities do
not share an image; the overlay and generation publication model below are the
next implementation phase after sealed-image measurements.
