...
### Level B Token and AST foundation

Level L must provide `Token` and AST class families implemented entirely in
Level B. These classes are a bootstrap dependency and therefore must not import
Level G or rely on a Unicode library generated by Level L. They may use the
existing Level B valid-UTF-8/codepoint and binary facilities. A completed Level
L tool may generate data and source consumed by Level G without introducing the
reverse dependency.

The minimum logical `Token` contract is:

- deterministic numeric or symbolic token kind;
- zero-based source byte start; and
- source byte length.

The minimum logical AST-node contract is:

- stable node or production kind;
- zero-based source byte start and byte length;
- ordered children; and
- an association with the originating `Token` for a terminal leaf.

This fixes observable structure, not physical layout. The bootstrap may use
ordinary Level B objects, arrays, and linear lookup. Generated implementations
may later use packed token streams, packed node records, child-index pools, or
hybrid representations, provided they expose the same ordered tree and source
information. Exact class/module names, ownership, and mutation APIs are
implementation decisions to be proved by the first frontend.
...
A successful generated parse will construct a syntactic AST automatically:

- each named production creates a node identified by that production;
- recognized terminal and nonterminal results become ordered children;
- terminal leaves retain their `Token` association and source span; and
- each nonterminal node spans from its first through its last recognized child.

No embedded production action is required to obtain this tree. A later Level B
visitor or tree-rewrite pass may construct a typed semantic AST or the neutral
Level L model. Listener, visitor, typed-tree, and rewrite facilities may be
introduced after concrete use cases establish their contracts; they are not
part of the bootstrap dependency.
...
- typed parser actions, semantic-tree helpers, visitors/rewrites, or recovery;
  or
- physical token/AST/table layout.
...
1. the hand-crafted Level B frontend parses the canonical Level L lexer
   specification;
2. the Level L generator emits a replacement lexer as deterministic Level B
   source;
3. the replacement lexer is substituted under the bootstrap parser and compared
   with the bootstrap lexer for tokens, spans, diagnostics, and the resulting
   canonical AST dump;
4. after parser syntax is exercised and approved, the same approach generates
   the Level L parser, whose reductions automatically construct AST nodes; and
5. successive generated frontends must regenerate byte-identical source and
   equivalent canonical ASTs before the generated frontend becomes primary.

Committed canonical specifications and generated Level B source break the
bootstrap cycle. A clean checkout starts with the existing `rxc`, `rxas`, and
VM, builds the retained bootstrap source, regenerates the frontend, and checks
the deterministic fixed point. No opaque prebuilt Level L binary is required.
