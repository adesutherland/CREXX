...
| `rxstats` | Level G standard/default | native provider over borrowed `.packedfloat` payloads; scalar operations/accessors process-reentrant, C object publication session-affine | Statistics is Level G standard functionality, outside language/bootstrap closure. RCC-5F replaced the boxed surface without compatibility overloads. The 15 September RXPA cleanup moves the immutable `linearfit` carrier into C, preserving its public API and calculations; see [QA01-AC-05](../native-inference-step-04.md). |
