...
| `Os` adapter | Rexx wrapper + native `system` plugin | T2 registered | D2 | medium-high | Platform behavior and the very broad system plugin are the main risks. It should be reviewed with the plugin, not as an isolated pure-Rexx class. |
...
| `crexx` driver | pure Level B source, C-packed and hybrid through system integration and native tool invocation | T2 | D2 | medium | Focused driver/native-packaging cases passed. It is a bootstrap self-hosting proof and a user entry point, so quoting, path, failure propagation, and cross-platform tests are important. |
| `rxvm`/`rxbvm`, `rxvme`/`rxbvme` | pure native VM cores; packed variants include Level B bytecode | T3 | D3 | high | Extensive instruction/runtime suites and two dispatch forms provide strong functional evidence. Dispatch, allocation, strings, calls, decimal, sockets, profiling hooks, and library packing all require measured release baselines. |
| `rxvml`/`rxbvml` | pure native embeddable runtime libraries | T3 through VM and native packaging | D3 | high | Same core risks as executables plus public host ownership/error contracts. Static delivery is intentionally part of native packaging. |
...
| rxmath | mixed, native math/stat/hash functions | T2 registered | D1 | medium | Broad mixed-purpose API has smoke evidence but needs numerical/vector tests per function family; it also conflicts in name with old unbuilt sources. | high |
...
| system | mixed, broad platform C | T2; passed focused run | D1-D2 | high | First-party static dependency but very broad API. Manual allocation, fixed-format buffers, directory/process/clipboard/global state, and platform branches warrant subsystem-level tests and hardening. | high |
