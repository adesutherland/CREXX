...
\item[BRTPANDT     {ID,REG,INT}         ]        if op2 readable status flags \& op3 true then goto op1
\item[BRTPT        {ID,REG}             ]        if op2 has externally writable status flags then goto op1
...
\item[GETTP        {REG,REG}            ]        gets the readable register status flags (op1 = op2.flags)
...
\item[LOADSETTP    {REG,INT,INT}        ]        load register and sets externally writable status flags (op1.flags = op3)
\item[LOADSETTP    {REG,FLOAT,INT}      ]        load register and sets externally writable status flags (op1.flags = op3)
\item[LOADSETTP    {REG,STRING,INT}     ]        load register and sets externally writable status flags (op1.flags = op3)
...
\item[SETORTP      {REG,INT}            ]        or externally writable register status flags (op1.flags = op1.flags | op2)
\item[SETTP        {REG,INT}            ]        sets externally writable register status flags, preserving VM-private flags
...
\item[SUBSTRING    {REG,REG,REG,REG}    ]        String slice op1 = op2[op3..op3+op4)
