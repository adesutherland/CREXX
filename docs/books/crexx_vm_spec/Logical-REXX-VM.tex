...
that can be used to dynamically store value status. The flag word is
partitioned: low bits are VM-private/read-only outside the VM, the next byte is
compiler call ABI state, and higher bands are reserved for library/runtime and
