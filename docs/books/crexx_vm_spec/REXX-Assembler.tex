...
settp - sets externally writable register status flags, preserving VM-private flags
setortp - or externally writable register status flags (op1.flags = op1.flags | op2)
brtpt - if op2 has externally writable status flags then goto op1
brtpandt - if op2 readable status flags & op3 true then goto op1
\end{verbatim}

The status flag word is 32-bit storage: the low byte is VM-private/read-only
outside the VM, the next byte is compiler call ABI state, and higher bands are
reserved for library/runtime and user contracts
...
\texttt{0x00000100}. RXAS instructions set public flag bands explicitly; this is
not an automatic runtime type-validation mechanism. VM-private cache flags are
