...
This section provides an overview of the optional, experimental GTK 3 plugin
for cREXX. `lib/plugins/gui/gui.c` implements the RXPA provider,
`gui_sample1.crexx` is the larger demonstration, and `gui_test.crexx` is the
focused automated/interactive contract test.
