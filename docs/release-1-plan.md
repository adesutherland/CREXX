...
| 19 | Add performance benchmark baseline for Release 1 | Rene | `rel1`, `must`, `performance`, `tests` | Implemented for beta 3 WIP with serial median sampling, opt/noopt and rxvm/rxbvm comparisons, separate tool timing, sanitizer coverage, and the Linux ARM64 report in `docs/planning/beta-3/reports/linux-vm-sanitizer-performance-review.md`; cross-platform counters remain follow-up. |
