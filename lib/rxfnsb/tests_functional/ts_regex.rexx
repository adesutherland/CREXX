...
/* ##array string tx AR*/
tx = REGEXFINDALL(s, "\d+")
call TEST_EQ 'FINDALL count digits', tx.0, 2
call TEST_EQ 'FINDALL tx.1', tx.1, "7"
call TEST_EQ 'FINDALL tx.2', tx.2, "42"
/* 3) REGEXSPLIT (gaps) */
/* ##array string parts AR*/
parts=.string[];
parts = REGEXSPLIT("a,,c,", ",+")
call TEST_EQ 'SPLIT count a,,c,', parts.0, 3
...
details=regexdetails() ; call TEST_EQ 'substr [0-9]', substr("1,2,3,4", details.1, details.2), "1"
/* 6) REPLACE_LIMIT */
...
details=regexdetails()   ; call TEST_EQ '(?i) matched whole file', substr("File.Txt", details.1, details.2), "File.Txt"
...
s = "ant bat cat"
/* ##array string tx2 AR*/
tx2 = REGEXFINDALL(s, "cat|ant")
call TEST_EQ 'alt order ant then cat count', tx2.0, 2
...
details= regexdetails(); call TEST_EQ 'class fast len', details.2, 1
...
details=regexdetails() ; call TEST_EQ 'MiXeD123 len', details.2, 8
/* 14) Overlapping FINDALL */
/* ##array string t3 AR*/
t3 = REGEXFINDALL("aaaa","aa", "O")
call TEST_EQ 'overlap count', t3.0, 3
...
a4 = REGEXSPLIT("  a , , b  ", "\s*,\s*", "TD")
call EXPECT_STEM 'SPLIT TD trim/drop', "a|b", a4
/* 19) FINDALL limit */
/* ##array string t4 AR*/

t4 = REGEXFINDALL("a1b2c3", "\d", '', 2)
call TEST_EQ 'FINDALL limit count', t4.0, 2
...
details=regexdetails() ; call TEST_EQ 'regexfind bar start', details.1, 3
/* 21) regexfind class fast-path from offset */
call TEST_TRUE 'regexfind [0-9] from 3 in a1b2', regexfind("a1b2","[0-9]",3)
details=regexdetails()  ; call TEST_EQ 'start digit at 4', details.1, 4
...
tx = REGEXFINDALL(s, "\d+")  /* text-only is enough for smoke */
call TEST_EQ 'findall count', tx.0, 2
