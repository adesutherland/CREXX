...
1. **Using Homebrew**
   ...
   brew install sqliteodbc             # SQLite driver and local regression DB
   brew install mysql-connector-odbc  # For MySQL
   ```

   `sqliteodbc` already depends on unixODBC, so `brew install sqliteodbc` is
   sufficient when SQLite is the only required driver.
...
3. **CREXX real-driver regression**
   ```bash
   ...
   With `BUILD_TESTING` enabled, CREXX always runs the deterministic mock ODBC
   tests. If CMake finds `sqlite3odbc`, it also creates a build-tree-only
   `CREXXSQLiteTest` DSN using SQLite `:memory:` and runs the real driver through
   both `rxbvm` and `rxtvm`. Override detection with
   `-DCREXX_SQLITE_ODBC_DRIVER=/absolute/path/to/the/driver` when necessary.
...
2. **PostgreSQL Specific**
   ...
   /* Use portable ODBC parameter markers */
   statement = odbc_prepare("SELECT * FROM mytable WHERE id = ?")
   rc = odbc_bind_integer(statement, 1, 42)
   rc = odbc_execute_prepared(statement)
   ```
...
#### odbc_database
```rexx
rc = odbc_database("mydb")
```
Sets the ODBC current-catalogue connection attribute.
...
- newdb: Name of database/catalogue to switch to

Returns:

- `0`: success
- `-1`: no connection or the driver rejected the catalogue change
