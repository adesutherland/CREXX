...
positive 1-based character position and defaults to one. An omitted `length`
uses the character length of `new`; a supplied zero writes no new text. A target
shorter than the characters preceding `start` and an overlay shorter than
`length` are extended with the one-character pad, which defaults to blank.
...
The implementation works on extracted RexxValue text, caches character
lengths, and uses direct VM substring, append, and pad operations. It calls no
name dispatcher or Level B formatting helper.
