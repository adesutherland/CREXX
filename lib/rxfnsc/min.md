...
```rexx
.rexxclassicbifmin..rexxclassicbif_min(
    context = reference .RexxBifCallContext) = .RexxValue
```

It implements variadic `MIN(number, ...)`. At least one argument is required;
...
comparison pass. It calls neither the name controller nor the Level B MIN
helper and allocates no comparison substrings.

`lib/rxfnsc/tests_functional/testRexxClassicBifMin.crexx` calls the standalone
