# cREXX PERF3 historical programme ledger
...
This file moved from `performance/ROADMAP.md` on 2026-09-04 so the canonical
path could become a compact live performance companion. Use the current
[`performance/ROADMAP.md`](ROADMAP.md) for selection and closeout status; use
this ledger only for PERF3 history and evidence provenance.
...
| VALUE-CACHE-01 | P2 architecture spike | Broad value-derived cache and invalidation design | numeric census not started; no numeric cache selected | RXAS M01 component/value proof remains V0 and is expected to remove most repeated conversions. The first numeric gate is a post-M01 residual census of same-source, same-context conversion reuse; only material residual hits justify a runtime PoC. The compact candidate uses a worker-local 32-bit monotonically increasing numeric-context ID: zero is uncacheable, IDs are never reused or wrapped, and saturation safely disables further caching for that worker. Shared atomic versions are rejected under current worker ownership. The intrinsic-string branch was separately selected and implemented as `UNICODE-CERT-01`; it does not authorize numeric provenance, a context field, value-layout growth or a numeric runtime cache. Control: [`VALUE-CACHE-01-WORKLIST.md`](VALUE-CACHE-01-WORKLIST.md). |
...
| LEVEL-L-INLINE-01 | P1 experimental | Cross-file inline storage proof for Level L workloads | published to hotfix/develop; local QA complete; hosted exact-SHA QA pending | I7 and its QA repairs provide structural receiver-storage proofs for exact non-reference concrete objects, direct indexed non-reference arrays, bounded nonescaping child-binary access and exact residual member dependencies. Local and imported methods use the same proof. Read-only expressions are evaluated once; statement-owned indexed mutation captures a live locator once and writes through to the original storage, with normal and signal cleanup. References, escaping/whole-value replacement, interface/dynamic dispatch, receiver-producing mutating expressions and unproved shapes remain ordinary calls. Focused optimized/no-opt, RXDAS and both-VM proof passes; the complete cumulative product evidence is recorded in `docs/ROADMAP.md`. The retained 2026-08-26 profiling-off Release bundle has valid manifest/runner hashes, 2 warmups plus 12 balanced pairs per VM, all checksums passing, and paired median improvements of 65.42% on `rxtvm` and 64.47% on `rxbvm`, with 12/12 favourable pairs in both cells. No formal benchmark was repeated after the correctness closeout: later changes remove unintended deep receiver copies and repair ownership/cleanup, so the retained bundle remains mechanism evidence, not an exact-final-SHA timing claim. Evidence: [`2026-08-26-level-l-inline-storage-first-release-verdict`](evidence/2026-08-26-level-l-inline-storage-first-release-verdict/). Code SHA `c0ac864d59428a807102a7b266933967f3b2e294` is published development content, not released/tagged content. |
| LEVEL-L-INLINE-02 | P1 experimental | Cross-file inline association transport | queued design/proof | Same-file cloning preserves compiler-generated `BLOCK_EXPR`/`LEAVE_WITH` and loop-control associations, while inline metadata currently rejects general associations. Design versioned association IDs, exact in-payload target ownership and reader-side topology validation before opening `BLOCK_EXPR`, `LEAVE_WITH`, `LEAVE` or `ITERATE`; keep each association family independently gated. |
| LEVEL-L-INLINE-03 | P2 experimental | Cross-file inline AST codec parity | queued survivor audit | Audit which locally inlineable nodes actually survive lowering into exported bodies before widening the whitelist. Decimal/numeric-control `DEC_*` nodes are the first plausible low-risk slice. Do not add `SELECT`/`WHEN` merely because they are absent from the codec: first prove that a useful post-lowering payload reaches that gate, then add focused source, RXAS/RXDAS, binary-import and runtime coverage. |
| LEVEL-L-INLINE-04 | P1 design | Imported factory and wider receiver-storage semantics | queued semantic design | Keep imported factories, residual `FACTORY_CALL`, whole aggregate replacement/return, escaping child `.binary`, reference-bearing objects, multidimensional arrays/stems and dynamic/interface dispatch closed. Any future slice must separately prove selector/match identity, factory initialization and lifetime, aggregate copy/alias/escape behavior, exact class layout and receiver copyback; do not open these as a general codec relaxation. |
| LEVEL-B-CALL-POSTFIX | P2 language/parser | Computed-receiver standalone `CALL` surface | published to hotfix/develop; local QA complete; hosted exact-SHA QA pending | The dedicated Level B `CALL` grammar has a zero-conflict member-postfix target which requires a final method suffix. Indexed, parenthesized, factory/function-produced and chained receivers are admitted without treating arbitrary expressions as statements or changing classic `CALL name arg,...`. Lemon regeneration reports zero conflicts. Optimized/no-opt structural and runtime coverage proves evaluate-once indexed receivers, retained mutation write-through and no residual target `setName()`/`setAndReport()` call; both images assemble and run under `rxtvm` and `rxbvm`. Code SHA `c0ac864d59428a807102a7b266933967f3b2e294` is published development content, not released/tagged content. |
| RXC-CALL-REG-01 | P1 compiler defect | Statement-call and temporary member-receiver register lifetime | published to hotfix/develop; local QA complete; hosted exact-SHA QA pending | Thousands of mutually exclusive generated call sites exposed three allocator lifetime omissions: discarded standalone-call results were retained, temporary member receivers outside the reserved call window were retained, and a statement call's node register was returned too late to be reused by deferred cleanup. The repair recycles those temporaries without changing call-window ownership. The permanent 32-call regression compiles, assembles and runs under both VMs in optimized and no-opt modes; the dispatch procedure uses 6 and 39 locals respectively, within the 7/39 limits, while the retained pre-fix no-opt image required 101. The Unicode 3,440-state diagnostic remains scale evidence only: reducing its scratch `normalize()` frame from 6,455 to 361 locals did not materially improve wall time, so this is a compiler resource repair rather than a generated-normalizer performance claim. Code SHA `c0ac864d59428a807102a7b266933967f3b2e294` is published development content, not released/tagged content. |
...
| KEYACCESS-01 | P1 independent | KeyAccess bulk-insert scaling | in progress — incremental hash-index maintenance under investigation | The current unique-write path linearly scans the index for every key, producing O(n²) loading. Compare the existing lazy hash index maintained incrementally against the status quo, preserving replacement, delete, rollback, reopen, cache and traversal semantics. Control: [`KEYACCESS-01-WORKLIST.md`](KEYACCESS-01-WORKLIST.md). |
| KEYACCESS-02 | P1 independent | KeyAccess negative-lookup scaling | first Release verdict complete — awaiting review | At large index sizes, ordinary missing reads spend measured time in per-miss error logging, not in a linear index fallback. Suppress logging for expected `NOT_FOUND` results while retaining full-key collision checks and all failure logging. Control: [`KEYACCESS-02-WORKLIST.md`](KEYACCESS-02-WORKLIST.md). |
...
| RXVECTOR-01 | P1 approved capability | Exact packed vector CPU provider and `crexx-rag` acceleration qualification | first Release verdict accepted; macOS closeout complete | The process-reentrant Level G `rxvector` provider passes the exact contract and reaches median public kernel times of 8.431/8.486 ms on `rxbvm`/`rxtvm`: paired median speedups of 15.62x/14.68x over the Level B oracle, at the direct shared-kernel ceiling. All 72 governed samples pass with the exact checksum, and Adrian accepted the result on 2026-08-22. Scratch install proves automatic dynamic/static selection with benchmark-only providers excluded; public documentation, bounded-page downstream cutover, and proportional normal-Debug QA are complete. Supported Linux sanitizer and cross-platform qualification remain later release QA. Control: [`RXVECTOR-01-WORKLIST.md`](RXVECTOR-01-WORKLIST.md); evidence: [`2026-08-22-rxvector01-first-release-verdict`](evidence/2026-08-22-rxvector01-first-release-verdict/). |
...
| UNICODE-NORM-02 | P2 experimental evidence | Generated prepared UTF-8 NFD DFA attribution | scratch attribution corrected; current generated lowering remains rejected | The Unicode research route prepares the Unicode 17 scalar partition, emits re2c rules, and lowers the resulting 3,440-state UTF-8 DFA to ordinary Level B loop/`SELECT` control flow. Follow-up work retracts the initial claim that the generated method's 6,455 locals explain its short-row cost: a scratch RXC allocation repair reduces `normalize()` to 361 locals without a material wall-time improvement, and profiling confirms that VM frame recycling is active. RXAS auto-selects ACPH because `JUMPI` integer keys serialize as eight bytes; the selector does not recognize numeric magnitude or dense range, but a matched forced comparison validates ACPH among the three current algorithms. At 3,440 cases, median `rxtvm`/`rxbvm` lookup deltas in ns were: first case, linear 11.672/12.311, open hash 16.382/16.403, ACPH 16.934/17.659; uniform cases, linear 2597.267/2872.267, open hash 25.712/24.167, ACPH 20.887/22.460; last case, linear 5225.900/5254.933, open hash 19.425/19.222, ACPH 17.253/16.077. The complete instrumented generated-row run executes 316,872 `JUMPI` operations with 24.492 ms aggregate diagnostic time, so neither `JUMPI` nor ACPH accounts for the large loss. The actual defect is the generated Level L `status()` forwarding method: across correctness plus one timed pass it is activated 40,069 times and its inlined Level B getter materializes the `_normalizer` class-attribute receiver with a whole-value `COPY_REG_REG`. Removing only the timed pass check deletes exactly 20,034 status activations and 20,034 whole-value copies while leaving all normalization-engine and `JUMPI` counts unchanged. In the paired timing profile, generated-row elapsed falls 3298.997 -> 2631.661 ms; the removed status bodies account for 479.645 ms and aggregate `COPY_REG_REG` time falls 476.278 ms. A three-sample balanced/interleaved ordinary profiling-off Release comparison retains checksum 55,422 and falls 1,539,936 -> 1,114,039 us on `rxtvm` (-425,897 us, -27.66%) and 1,540,852 -> 1,115,490 us on `rxbvm` (-425,362 us, -27.61%). The no-status image is a diagnostic control, not a product remedy: all compared normalizers perform the same per-row API check, and that reasonable check should remain cheap. These measurements are directional scratch evidence on the AC-powered Apple host, not a formal product verdict; the raw profiles and harnesses remain under `/tmp/crexx-unicode-regalloc.QDYoCK/` and must be promoted into a retained evidence bundle before implementation selection or a release claim. |
| UNICODE-NORM-02-F1 | P1 compiler follow-up | Exact read-only scalar accessor through a class-attribute receiver | roadmap evidence gate — no implementation authorized | Classify the `status()` finding as a conservative but materially costly RXC inliner boundary: it is not a correctness defect in the accessor and not an unreasonable hot-path call. The existing exact scalar-accessor proof recognizes the read-only getter, but receiver sharing deliberately excludes class attributes and therefore detaches `_normalizer` with a whole-object value copy. The bounded target is a scoped receiver-storage alias, not a persistent CREXX `reference`: `linkattr1 receiver,a1,1; assertinitialized receiver; linkattr1 slot,receiver,41; igetunlink result,slot; unlink receiver`. Admit initially only an exact, call-free, one-return scalar getter with no receiver write, reference escape, computed receiver, or copyback requirement. The proof must preserve evaluate-once ordering, `OBJECT_NOT_INITIALIZED`, storage identity, normal and signal-path link cleanup, source/TRACE identity, local and imported inline payloads, optimized/noopt equivalence, and both VM modes. Before a production edit, add a minimal compiler reproducer and compare the hand-equivalent RXAS ceiling, the current copied-receiver target, and the ordinary retained call. After minimum correctness, follow the mandatory ordinary profiling-off Release verdict with the real status check retained and unrelated representative guard cells; do not broaden this into generic class-attribute receiver sharing without a separate alias/copyback proof. |
...
| RCC5-MATH | P1 approved production | Level G mathematics composition and historical provider split | complete and published 2026-08-22 | The Level G standard family separates process-reentrant native `rxfloat` and packed `rxstats` from Level-B-authored `rxint` and `rxdecimal`. Direct scalar `rxmath` compatibility names share `rxfloat`; the mixed provider is retired. RCC-5D established a boxed statistics oracle and RCC-5F then removed that pre-release production surface in favor of direct read-only `.packedfloat` payload borrowing plus immutable `.linearfit`. The packed Release gate is clear favorable: paired median call rate improves 2054.025%/2062.894% on `rxbvm`/`rxtvm`, every retained pair is favorable, exact offset results match, and the public path remains about 91% of the direct native control. RCC-5E moves retained checksum/hash, ID, filesystem and platform contracts into `rx_hash`, `rxid`, `rxfs`, and `rxplatform`, removing draft developer/UI/legacy surfaces. RCC-5B's black-box suite covers all 37 float procedures and RCC-5C qualifies exact integer and decimal behavior through 128 caller digits without establishing a runtime ceiling. The consolidated current Release panel passes 19/19, normal Debug has broad-plus-targeted closure over all 2,356 cells, and complete Apple-ASan passes 2,356/2,356. Adrian assigned supported Linux ASan/LSan for live SAN-001 and SAN-002 to RCC-8 release QA; they remain release-blocking and no cross-platform sanitizer-clean claim is made. Control: [`RCC5-MATH-WORKLIST.md`](RCC5-MATH-WORKLIST.md); packed verdict: [`2026-08-22-rcc5f-packed-stats-first-release-verdict`](evidence/2026-08-22-rcc5f-packed-stats-first-release-verdict/); RCC-5D/E verdict: [`2026-08-21-rcc5de-provider-split-first-release-verdict`](evidence/2026-08-21-rcc5de-provider-split-first-release-verdict/); SAN worklist: [`SANITIZER-WORKLIST.md`](../docs/SANITIZER-WORKLIST.md); validation: [`mathematics-validation-strategy.md`](../docs/planning/release-1/mathematics-validation-strategy.md); packed prerequisite: [`BINARY-01-WORKLIST.md`](BINARY-01-WORKLIST.md). |
| UNICODE-NFD-01 | P2 experimental evidence | Portable fixed-width versus host-native packed NFD tables, with UTF-8 Apple CoreFoundation controls | clean AC baseline captured; portable default retained | The 2026-08-25 user-confirmed-clear AC session passes the complete Unicode 17.0.0 gate under both optimized Release VMs, with two warmups, ten serial samples, raw samples, peak RSS and pre/post host state. Single-call UTF-8 buffers safely join the corpus with CCC-zero U+0020 and repeat it 1, 8 and 64 times. Throughput stays nearly constant across 101,506 to 6,496,384 input bytes: portable sustains 1.61–1.75 MB/s and packed 1.80–1.94 MB/s across the two VMs. Packed is an observed 9.7–11.2% faster on sustained buffers and 5.6–6.7% faster on 20,034 separate rows, but its 9,074,136-byte table is 7.65 times the 1,186,472-byte portable image. This is not a paired-interleaved format-selection verdict; portable therefore remains the default while hybrid/packed representations stay measured alternatives after algorithm work. CoreFoundation misses two Unicode 17 rows and has pathological large-buffer scaling through the current RXPA string bridge, so it remains a bounded sense check. The prepared four-form image, hand-written engine, and generated re2c NFD successor all pass their complete conformance gates. The experimental RXAS additions have been fully rolled back; the later product baseline selects one shared prepared constant and RXVM codepoint execution; this row remains the historical format comparison. Retained decision evidence: [`2026-08-28 Unicode normalization decisions`](evidence/2026-08-28-unicode-normalization-decisions/). |
| UNICODE-NFD-02 | P2 experimental implementation | Prepared codepoint NFD `normalize` and `is_normalized` string family | integrated four-form conformance and directional Release gate green | The NFD `.string` family remains entirely in the RXVM Unicode domain. `normalize` iterates with `STRCHAR`, reads the single shared prepared constant with `BGETU32`, and emits with `APPENDCHAR`; it does not pass through `.binary`. The exact D-form predicate reads the same prepared word per codepoint, rejects mappings and Hangul syllables, and checks canonical-combining-class order without constructing normalized output. The combined optimized/noopt harness passes all 400,680 normalization relations and 400,680 predicate checks on both VMs. The experimental binary methods and dead legacy `lexer_*` byte-scanner hooks remain outside this decision. This evidence underlies the integrated Level G four-form product path. |
| UNICODE-NORM-03 | P2 experimental implementation | Prepared codepoint NFD/NFKD/NFC/NFKC string family and exact predicates | canonical Quick_Check correction green; directional Release rerun complete | All four `normalize(.string)` forms use `STRCHAR`, direct reads from one 11,237,664-byte compiler-emitted constant and `APPENDCHAR`, with no `.string`/`.binary` conversion or per-normalizer table copy. The image now includes packed official Unicode 17 NFC_QC/NFKC_QC data. C-form predicates reject `No`, accept an ordered all-`Yes` scan and invoke the exact allocation-free streamer only for `Maybe`; D forms retain their direct decomposition/order checks. The optimized/noopt dual-VM gate passes 400,680 normalization relations plus 400,680 exact predicate checks, including a focused LVT-Hangul-plus-Maybe fallback regression, and RXAS shows one constant, no normalizer image attribute and no output/conversion operation in `is_normalized`. On the mostly clear AC host, mixed-row shared NFC/NFKC predicates improve 3.87–4.25x. Known-normalized joined buffers contain `Maybe`, so the canonical preliminary scan plus whole-input fallback is 9.9–13.4% slower than the old exact-only path. A local-Maybe/stable-region successor is a separate algorithm experiment. Full directional evidence: [`2026-08-28 common normalization`](evidence/2026-08-28-unicode-normalization-decisions/2026-08-28-common-normalization-directional.txt). The integrated product baseline selects this architecture and keeps local-Maybe optimization as follow-on work. |
| UNICODE-CERT-01 | P1 approved production | Four intrinsic `.string` normalization certificates | implementation frozen; informal product-VM screen green; formal clean-host verdict not run | C1 is implemented: the former compiler byte is split logically into two call-ABI bits, four language-owned NFC/NFD/NFKC/NFKD bits and two reserved language bits without changing the status word or `value` layout. VM string mutation clears all four; exact whole-string copies and owner-local moves preserve them; empty and known-ASCII production set all four. Compiler call setup preserves the language sub-band, while `.flags.language` is read-only to Level B. Focused Debug tests, RXPA opaque-mutation invalidation, dual-VM optimized/no-opt 400,680-relation conformance and RXAS shape checks are green. The battery-powered product-`rxbvm` screen retained zero mismatches: large-buffer normalization was 2.33–5.83% faster and cold predicates ranged from 1.72% faster to 3.79% slower than the earlier directional run; certified hits improve normalization 26.1–133.8x and predicates 15.0–166.1x. This is adequate regression evidence for the documentation checkpoint, not a formal clean-host or dual-VM verdict. Numeric conversion caching remains independently gated by `VALUE-CACHE-01`; non-ASCII cross-worker materialization remains conservatively unknown. Control: [`UNICODE-CERT-01-WORKLIST.md`](UNICODE-CERT-01-WORKLIST.md); evidence: [`2026-08-28 informal certificate screen`](evidence/2026-08-28-unicode-normalization-decisions/2026-08-28-normalization-certificate-informal.txt). |
| UNICODE-GRAPHEME-01 | P1 approved production | Unicode 17 default extended-grapheme surface and prepared property image | integrated and qualified in the product baseline | UAX #29 revision 47 default extended grapheme clusters are an explicit Level G `.string` facility. Direct count and bounded substring use one codepoint-streaming scanner; the explicit immutable `graphemes` view builds one private packed codepoint-boundary index and reuses it for indexed access, position, reverse and iteration. One compiler-owned read-only 1,114,176-byte property constant packs GCB, Extended_Pictographic and InCB; the executor reads it directly with `STRCHAR`/`BGETU8`, with no re2c UTF-8 decoder, UTF-32 conversion, `.binary` round trip, runtime table copy, normalization, case folding or VM flag. The complete Unicode 17 conformance/shape gate passes optimized and no-opt on both VMs. The accepted ordinary profiling-off Release verdict uses a 49,152-byte/7,168-grapheme buffer and 12 balanced/interleaved observations per cell: per-call full indexing is 1.802-2.953% slower than streaming across count/substring and both VMs, so direct operations retain streaming while the reusable view retains explicit indexing. Product error paths use the Level B `SIGNAL` instruction; source-level assembler signalling and the redundant `raise` wrapper are rejected. Clean Debug and profiling-off Release each pass 2,412/2,412; the maintained Apple-ASan suite passes 2,412/2,412 with the documented Apple leak-detection limitation. A new runtime string/grapheme type, implicit ordinary-BIF changes and mutable/shared grapheme caches remain outside this stage. Evidence: [`2026-08-29 grapheme first-Release verdict`](evidence/2026-08-29-unicode-grapheme-first-release-verdict/). |
| UNICODE-NORM-02 | P2 experimental evidence | Original scalar NFD versus generated prepared UTF-8 DFA, with hand-prepared controls | rough Release screen complete; current generated lowering rejected | The generated route converts the prepared Unicode scalar partition to re2c rules, a 3,440-state UTF-8 DFA, and ordinary Level B loop/`SELECT` control flow. Optimized and noopt images pass 100,170 NFD corpus relations plus 1,094,978 unlisted-scalar identity checks identically on both VMs. The AC rough Release screen uses the same `.string -> .string` surface, one warmup and three serial samples. Generated NFD is 84.010x/101.059x slower than original on 20,034 rows and 8.974x/8.818x slower on one 101,506-byte buffer for `rxtvm`/`rxbvm`. Immediate hand-prepared context cells show the generated large-buffer path is also 1.086x/1.032x slower. Optimized RXAS contains 73 `JUMPI` dispatches, but the method declares 6,455 locals; recycled VM frame activation relinks every local per call, explaining the amplified short-row cost. This rejects the current Level B DFA code shape, not the prepared partition proof. Any successor must bound live/register state independently of DFA size and reduce the shared output/order engine before another timing claim. Exact retained result: [`generated NFD rough screen`](evidence/2026-08-28-unicode-normalization-decisions/2026-08-26-generated-nfd-rough.txt). |
