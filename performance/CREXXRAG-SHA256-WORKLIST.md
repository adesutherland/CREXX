...
Status: comparative prototype complete; Option A selected by Adrian on
2026-08-19; production API and deployment architecture await approval; no
production SHA-256/API change authorized or made
...
This is a focused, explicitly non-representative capability experiment. It
does not alter the common performance portfolio and does not authorize a public
API, ABI, plugin-package, or production-library change.

## Selection

Adrian selected **A** on 2026-08-19. The public procedure signature is to be
published directly through RXPA; a Rexx wrapper is not required merely to make
the typed declaration visible to `rxc`. The remaining design decision is how a
compiled/linked image records and resolves its native provider so ordinary
`rxc`, `rxvm`, linked-image and native-package use does not depend on a
remembered command-line module list.
...
| G4 runtime capability composition | proposed — approval required | The [`Runtime Capability Composition Roadmap`](../docs/planning/release-1/runtime-capability-composition-roadmap.md) proposes `rx_hash` as standard/default rather than core, with declarative native-provider dependencies, static-first trusted autoload, automatic native-package linkage, and explicit module initialization before `main`. No Rexx declaration wrapper or remembered `rxvm` module list is the target design. |
