...
| `havlak` | `awfy_havlak.crexx` | pinned AWFY CFG, loop-recognition, union-find and loop-forest contracts through stable indexed Level B state | post-PERF3 qualified `stable-indexed-cfg-union-find` reserve lane | no |
...
- Keep Full AWFY Json separate from all three earlier JSON identities. Its
  indexed standard-library representation is disclosed, and it remains
...
- Keep Havlak outside every aggregate. Its direct executable retains the exact
  50 discarded recognition passes; bounded CTest and maintained-runner forms
  must report their explicit repeat count. Normal value-array and any future
  packed-binary controls remain separate identities.
...
Full AWFY Json, DeltaBlue, CD and Havlak completed their own post-PERF3
qualifications on 2026-08-17 and are reserve lanes, not promoted members.
