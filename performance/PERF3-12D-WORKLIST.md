...
**Status:** implementation and closeout QA complete in the isolated worktree;
direct reuse plus a semantically guarded `link` fallback is ready for review
...
QA is complete; commit and push remain separate.
...
The implementation remains uncommitted on the isolated branch. No sanitizer,
install/package, cross-platform or additional benchmark sweep was added beyond
the approved closeout path.
...
later binary/object values. Closeout QA is complete. The approval does not
by itself authorize a new public Pattern Program format, RXAS/RXBIN
instruction, regex/PEG syntax, commit, push or pull request.
