/* Focused raw-copy lifecycle proof for the DECIMAL-01 libmpdec candidate. */
...
int main(void) {
    ...
    value_init(&copy);
    value_init(&addend);
    if (load_rxvmplugin(".", "rxvm_mpd_decimal") != 0) {
    ...
    /* Destroy the original owner before using the byte-for-byte copy. */
    clear_value(&source);
    ...
    puts("PASS: libmpdec pointer-free lifecycle");
    return 0;
}
