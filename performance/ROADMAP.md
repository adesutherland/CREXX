...
| RCC5-MATH | P1 approved production | Level G mathematics composition and historical `rxmath` split | RCC-5A complete; RCC-5B implementation/verdict accepted with focused maths coverage open; RCC-5C in progress; RCC-5D+ not started | The Level G standard mathematics family separates a process-reentrant native `rxfloat` provider from Level-B-authored `rxint` and `rxdecimal` modules. `rxmath` scalar compatibility names share the native procedures; statistics waits for the separately approved native-packed `BINARY-01` representation. Correctness uses libm, Euclidean/divide-before-multiply/binary-search/modular algorithms and guarded decimal Newton/range-reduced series rather than the defective or dormant historical implementations. RCC-5 has one consolidated full-QA closeout after its final approved subphase; intermediate slices use focused correctness plus any mandatory first-Release verdict. Control: [`RCC5-MATH-WORKLIST.md`](RCC5-MATH-WORKLIST.md); RCC-5B evidence: [`2026-08-20-rcc5b-rxfloat-first-release-verdict`](evidence/2026-08-20-rcc5b-rxfloat-first-release-verdict/); packed prerequisite: [`BINARY-01-WORKLIST.md`](BINARY-01-WORKLIST.md). |
