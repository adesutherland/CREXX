...
| PERF3-12 | P1 | Current RexxCPS clause-lowering rereview | complete — K04e reassessed; PERF3-12A cursorless prerequisite in progress | Current fixed-count schema-5 profiles are checksum-valid under both VMs at 53.661M optimized instructions. K04e removes its exact 560k hot dispatches without changing any ranked mechanism count. The largest ceiling remains transactional PARSE placement at 9.24M dispatches; compound-tail routes expose 1.96M-2.24M concats. Adrian approved a breaking cursorless string/binary RXAS redesign before copied-XTOY placement: explicit-position operations replace logical value cursors, VM-private UTF cache state becomes unobservable, and removed value fields form a compile-time inventory fence. After its own first Release verdict, PERF3-12A continues against the retained 2.22M `DCOPY` plus 97.68 MB target. TRACE is guard-only and ADDRESS is zero-runtime here. Control: [`PERF3-12-WORKLIST.md`](PERF3-12-WORKLIST.md), [`PERF3-12A-WORKLIST.md`](PERF3-12A-WORKLIST.md); current evidence: [`2026-08-04-perf3-12-k04e-clause-reassessment`](evidence/2026-08-04-perf3-12-k04e-clause-reassessment/); original gate: [`2026-08-04-perf3-12-rexxcps-clause-rereview`](evidence/2026-08-04-perf3-12-rexxcps-clause-rereview/). |
...
cursorless product stops at its own mandatory ordinary Release verdict. Once
accepted, copied-XTOY placement resumes against the retained 2.22M
`DCOPY`/`DTOS` target and receives a separate verdict before combined closeout.
