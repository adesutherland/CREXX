...
| LEVEL-B-CALL-POSTFIX | P2 language/parser | Computed-receiver standalone `CALL` surface | temporary-branch implementation complete; focused proof passed; awaiting staging | The dedicated Level B `CALL` grammar now has a zero-conflict member-postfix target which requires a final method suffix. Indexed, parenthesized, factory/function-produced and chained receivers are admitted without treating arbitrary expressions as statements or changing classic `CALL name arg,...`. Focused optimized/no-opt runtime coverage proves the surface forms; the indexed receiver is evaluated once, mutation is copied back, and optimized RXAS contains no residual target method call. Broader regression and Level C remapping qualification remain staging work. |
