...
| CREXXRAG-SHA256 | P1 capability decision | Binary SHA-256 native RXPA versus pure-Level-B comparison | Option A selected 2026-08-19; composition decisions in progress; stopped before production | Correctness passes standard, embedded-zero and padding-boundary vectors on both concrete VMs. In the serial profiling-off Release pilot, including the required noise appends, integrated native is 196.5x-252.9x faster in-kernel than straightforward pure Level B, reaching 164-407 MiB/s versus 0.765-1.726 MiB/s; the bulk RXPA path is within about 3-7% of the direct-C ceiling. Adrian selected one public Level G-usable RXPA hash surface over a small process-reentrant `rx_hash` provider, not a monolithic all-assets native library; retain pure Level B only as reference/explicit fallback. Direct RXPA declarations need no Rexx wrapper. The [`Runtime Capability Composition Roadmap`](../docs/planning/release-1/runtime-capability-composition-roadmap.md) proposes `rx_hash` as standard/default rather than core, with declarative provider autoload, automatic native-package linkage and explicit module initialization. Its structural boundary is accepted: remain on 007 when the records use existing metadata homes; move to 008 for a file-structure change. The opcode disposition list and once-per-mutable-module-instance initializer semantics are accepted. No public API, ABI, package or production source changed. Control: [`CREXXRAG-SHA256-WORKLIST.md`](CREXXRAG-SHA256-WORKLIST.md); [evidence](evidence/2026-08-19-crexxrag-sha256-a-vs-d-gate/). |
