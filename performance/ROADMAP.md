...
| PERF3-04 | P1 | Generic final/concrete scalar accessor proof | active post-PERF3 successor | Full AWFY Json, DeltaBlue, CD and Havlak now provide the expanded proof suite. A hand-equivalent ceiling and generic compiler proof must precede any production candidate. `POSTPERF-04-PACKED-01` also compares portable checked `i64` with an explicitly selectable aligned `rxinteger` access path; alignment, extent, bounds and host representation are proved once, while syntax and backing storage remain open. No JSON- or vector-specific opcode is implied. |
