...
| PERFORMANCE-CLOSEOUT | P0 release evidence | Current-product performance closeout | Stages 1-4 complete; Stage 5 active | Portfolio v3 is source-reviewed, capability-classified, calibrated and green. The review repaired structured cleanup and nested-signal-handler correctness defects rather than weakening Havlak; 2,262/2,262 Debug tests, focused Release integrity and the dual-VM/cross-runtime qualification pass. The Stage 5 scorecard builds the frozen commit fresh and reports cREXX engines, qualified ooRexx/genuine-NetRexx cells and pinned Java/CPython controls separately. The historical NetRexx Base64 row is now explicitly a Java/JVM control, so v3 does not pretend that the cREXX/ooRexx common-five and genuine-NetRexx common-four are the same aggregate. Control: [`closeout plan`](PERFORMANCE-CLOSEOUT-PLAN.md); v3 contract: [`manifest-v3`](portfolio/manifest-v3.md); Stage 4 evidence: [`2026-08-18 closeout Stage 4`](evidence/2026-08-18-performance-closeout-stage4/). |
