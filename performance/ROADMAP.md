...
| LEVEL-L-INLINE-01 | P1 experimental | Cross-file inline storage proof for Level L workloads | accepted first Release verdict; isolated baseline checkpoint | I7 adds fail-closed structural receiver-storage proofs for exact non-reference concrete objects, direct indexed non-reference arrays, bounded nonescaping child-binary access, and exact residual member dependencies while preserving I6 for existing shapes. References, escaping/whole-value replacement, interface/dynamic dispatch and computed imported receivers remain ordinary calls. Focused source/binary, optimized/no-opt, RXDAS and dual-VM correctness passed. The 2026-08-26 profiling-off Release verdict retained 2 warmups plus 12 balanced pairs per VM with all checksums passing: paired median elapsed fell 65.42% on `rxtvm` and 64.47% on `rxbvm`, with 12/12 favourable pairs in both cells. Evidence: [`2026-08-26-level-l-inline-storage-first-release-verdict`](evidence/2026-08-26-level-l-inline-storage-first-release-verdict/). Keep staging separate from the occupied hotfix branch. |
