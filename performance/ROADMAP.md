...
| PERF3-05-R2 | P1 | Profile-selected RXAS/RXBIN instruction-handler inline/call framework | queued design and comparative PoC; no ISA change | Express every interpreter instruction handler once behind an internal macro/definition boundary that can emit a hot handler directly inside the dispatch owner or compile a cold handler as a normal callable function. Group handler definitions into coherent internal files for maintenance while preserving one semantic source. Select the inline set from dynamic instruction profiles plus native I-cache/branch/call evidence, not intuition; this may control `run()` size and limit accidental compiler macro-propagation or mega-function flattening. The PoC must define the called-handler state/continuation ABI and preserve PC movement, register/reference ownership, signals, interrupts, TRACE, profiling, native/plugin calls and exactly identical terminal behavior under portable switch and direct-threaded dispatch. Compare all-inline, profile-selected and deliberately cold-call controls for text/resident-hot size, call/branch cost, build time/RSS, artifact/RSS and representative end-to-end timing on both concrete VMs and representative compilers including MSVC. This extends PERF3-05-R1; it authorizes no public RXAS/RXBIN or plugin ABI change and no production selection before Adrian reviews the comparative evidence. |
...
| PERF3-13 / CAP-06 | P0 | RXVM allocator, value-shape and worker-communication foundation | Gate E E1-P1 accepted and locally complete; publication pending; E2 and Gate F closed | Gate C selected the 176-byte L32SDH value with co-allocated decimal length/capacity, and Gate D industrialised it with 64 KiB worker slabs, a 16 KiB standard-allocation ceiling, sticky sidecars and R0 explicit-only reclamation. EF-0 is published in `642e1b697` on synchronized base `19802842e`; spawn redirects now use immutable input and independent byte completions, with receiver-side register conversion after join. Adrian accepted E1 on 2026-08-07. A runtime now owns the memory context/depot and each compatibility VM embeds one thread-affine worker with a lock-free arena, owner checks, depth-counted execution and deterministic drain/stop. Its 48-pair pooled Release mean is -0.149146% (95% interval -0.780096% to +0.481804%) with no 3% guard; all 104 processes pass. Closeout passes focused ASan 3/3, full Debug 1,997/1,997, final Debug 13/13, 808 Release build steps and focused Release 14/14, including both concrete VMs and static archive linking. A post-closeout 60-pair RexxCPS review isolates a -0.780187% clear loss to lifecycle/error code inside flattened `run()`; a lifecycle wrapper is neutral against the post-EF-0 control and +0.657320% clear favorable against committed E1 while retaining the enlarged E1 context. Adrian accepted E1-P1 after its core-four first verdict was neutral on every workload: pooled -0.192153% (95% interval -0.632127% to +0.247821%) across 80 pairs and 168 passing processes. The wrapper restores the flattened core's register/stack pattern while retaining the enlarged E1 context. E1-P1 closeout passes focused ASan 3/3, full Debug 1,997/1,997 and ordinary Release 14/14. Apple LSan is unsupported; a broader ASan artifact build separately exposed a retained pre-existing RXAS use-after-free. Native Windows, Intel Linux and Linux ARM64 proof remains open. E2 requires separate approval; public channels and RXAS/RXBIN changes remain closed. The later Gate F value remains a logical scalar/binary register image with ordered child-register images, never an internal `value *`. Control: [`PERF3-13-WORKLIST.md`](PERF3-13-WORKLIST.md); E1-P1: [`2026-08-07-perf3-13-gate-e-e1-p1-wrapper`](evidence/2026-08-07-perf3-13-gate-e-e1-p1-wrapper/); E1: [`2026-08-07-perf3-13-gate-e-e1-worker-shell`](evidence/2026-08-07-perf3-13-gate-e-e1-worker-shell/); EF-0: [`2026-08-07-perf3-13-ef0-spawn-recovery`](evidence/2026-08-07-perf3-13-ef0-spawn-recovery/); Gate D: [`2026-08-07-perf3-13-gate-d-local-closeout`](evidence/2026-08-07-perf3-13-gate-d-local-closeout/); Gate C: [`2026-08-06-perf3-13-gate-c-m3-sidecar-decision`](evidence/2026-08-06-perf3-13-gate-c-m3-sidecar-decision/); Gate B: [`2026-08-06-perf3-13-gate-b-closeout`](evidence/2026-08-06-perf3-13-gate-b-closeout/). |
...
- `CAP-06` is active as PERF3-13. Gate A M0 audited every allocator-eligible
  ...
  full QA passes focused ASan 3/3, Debug 1,997/1,997 and Release 14/14. E2
  requires separate approval. Transport-neutral channel semantics stay closed
