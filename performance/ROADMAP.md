...
| UNICODE-NFD-01 | P2 experimental evidence | Portable fixed-width versus host-native packed NFD tables, with UTF-8 Apple CoreFoundation controls | clean AC baseline captured; prepared D correctness successor implemented; no product selection | The 2026-08-25 user-confirmed-clear AC session passes the complete Unicode 17.0.0 gate under both optimized Release VMs, with two warmups, ten serial samples, raw samples, peak RSS and pre/post host state. Single-call UTF-8 buffers safely join the corpus with CCC-zero U+0020 and repeat it 1, 8 and 64 times. Throughput stays nearly constant across 101,506 to 6,496,384 input bytes: portable sustains 1.61–1.75 MB/s and packed 1.80–1.94 MB/s across the two VMs. Packed is an observed 9.7–11.2% faster on sustained buffers and 5.6–6.7% faster on 20,034 separate rows, but its 9,074,136-byte table is 7.65 times the 1,186,472-byte portable image. This is not a paired-interleaved format-selection verdict; portable therefore remains the default while hybrid/packed representations stay measured alternatives after algorithm work. CoreFoundation misses two Unicode 17 rows and has pathological large-buffer scaling through the current RXPA string bridge, so it remains a bounded sense check. The 2026-08-26 branch-local successor prepares both NFD and NFKD into one 1,659,772-byte portable image, scans UTF-8 bytes without a whole-input copy, and passes 200,340 corpus relations plus 1,094,978 unlisted identities per form on both VMs in optimized and non-optimized images. Its `sblen`/`sgetu8`/`sbmove` opcodes and read-only exposed-input convention are experimental correctness/generated-shape evidence only; the `.string` result still has one final conversion copy. No Release timing comparison, machine-level primitive ceiling, production opcode, public binary format, or Level G API selection has been made. Control and result summaries: [`experiments/unicode/nfd-performance`](../experiments/unicode/nfd-performance/) and [`experiments/unicode/level-l-bootstrap`](../experiments/unicode/level-l-bootstrap/). |
