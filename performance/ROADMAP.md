...
| PERF3-05-R5 | P1 | Handler-placement percentage, never-inline and platform panel | stopped at explicit Apple GCC trade-off — no default selected | All 651 handlers now have one auditable heat/eligibility tier; 56 host-bound operations plus reserved/sentinel handlers stay callable in the 531/589 (90.15%) `max-eligible` panel. Literal all-inline expansion is exact and all requested percentages pass 1,120 Apple pilot executions. Formal Clang 20% is +3.857%/+3.152% for `rxtvm`/`rxbvm` with no guard and roughly 72% smaller owners; formal GCC is +3.175%/+9.646% but `rxtvm` Bounce regresses 10.072%. Every requested GCC non-inline panel fires a Bounce guard, so governance stops before a common default, Linux/Windows work or sanitizer expansion. Adrian must explicitly choose common 20% with that trade-off, compiler/engine-specific policy, further GCC threaded repair, or retained all-inline. Refresh the exact mix separately before release. Control: [`PERF3-05-R5-WORKLIST.md`](PERF3-05-R5-WORKLIST.md); evidence: [`2026-08-10-perf3-05-r5-handler-percentage-panel`](evidence/2026-08-10-perf3-05-r5-handler-percentage-panel/). |
