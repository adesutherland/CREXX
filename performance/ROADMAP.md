...
Industrial mailbox and logical-register completion semantics, Linux/Windows
proofs, E6, public workers/channels and Gate F remain separately gated.
...
- `CAP-06` is active as PERF3-13. Gate A M0 audited every allocator-eligible
  ...
  Mac Debug/ASan/Release closeout. The separate E5 macOS PoC now proves a native
  `pthread_kill` doorbell can reach the existing local interrupt word without a
  new hot-loop poll. Industrial E5/E6, Linux/Windows carrier proof and Gate F
  remain deferred. Transport-neutral channel semantics stay closed until the
  worker model is selected.
  Control:
