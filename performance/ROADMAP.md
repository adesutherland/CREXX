...
a settled, quiet, reserved host. Industrial mailbox and logical-register
completion semantics, Windows proof, E6, public workers/channels and Gate F
remain separately gated.
...
+5.69%/+5.07% in `rxtvml`; the E4 owner for non-targetable and native-capable
  ...
  default parser-enabled configuration. Industrial mailbox semantics, public
  workers/channels, E6 and Gate F remain separately gated.
...
- `CAP-06` is active as PERF3-13. Gate A M0 audited every allocator-eligible
  ...
  Mac Debug/ASan/Release closeout. The separate E5 POSIX PoC now proves on
  macOS and Intel Linux that a native `pthread_kill` doorbell can reach the
  existing local interrupt word without a new hot-loop poll. Linux GCC/Clang
  functional and generated-code proof passes; its stressed-host performance
  result is accepted as overall noisy/inconclusive at PoC precision.
  Industrial E5/E6, Windows carrier proof and Gate F remain deferred.
  Transport-neutral channel semantics stay closed until the worker model is
  selected.
  Control:
