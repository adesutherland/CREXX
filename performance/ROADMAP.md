...
| PERF3-05-R2 | P1 | Profile-selected RXAS/RXBIN instruction-handler inline/call framework | complete at approved Apple 30% checkpoint — framework retained, no production policy selected | All 649 opcode/sentinel handlers plus two private handlers now have one grouped macro definition that emits direct owner code or a force-noinline call under an explicit per-instruction policy; the RXAS/RXBIN and plugin ABIs are unchanged. All-inline, all-outline and the common 176/588 (29.93%) profile panel pass 2,002/2,002 Release tests; all-outline also passes 2,002/2,002 fresh Debug tests. The framework moves `run()` from roughly 532 KiB all-inline to 32 KiB all-outline or 200 KiB profile-30. A balanced 588-execution Release matrix passes every oracle, but profile-30 loses 9.35%/12.08% geometric-mean throughput on `rxtvm`/`rxbvm`; all-outline loses 40.02%/34.24%. Six workloads execute zero outlined profile-30 handlers and RexxCPS executes only 8 in roughly 23M instructions, proving that the profile-30 loss is compiler/layout shape rather than call overhead. Frequency identifies dynamic heat (29 handlers cover 75.07%; 176 cover 99.9999969%) but does not select a fast owner shape. Stop before production selection, VM-specific panels, layout optimization or cross-platform closure. Control: [`PERF3-05-R2-WORKLIST.md`](PERF3-05-R2-WORKLIST.md); evidence: [`2026-08-09-perf3-05-r2-handler-panel`](evidence/2026-08-09-perf3-05-r2-handler-panel/). |
| PERF3-05-R3 | P1 | Handler code-generation, interrupt-tail and cross-compiler defect analysis | in progress — approved through proved internal repairs and Clang/GCC remeasurement | Preserve R2 and determine why profile-30 regresses despite executing no outlined handlers in six governed workloads. Audit normalized expansion, pointer-facade escape, signal/interrupt polling and continuation CFG, helper inlining, opcode/label mapping and generated dispatch. Freeze untouched/all-inline/profile-30/all-outline products under Apple clang 21 and real Homebrew GCC 16.1, validate both `rxtvm` and `rxbvm`, then run a same-session four-shape GCC matrix before any fix. Use bounded wrapper-reachability, outlined-site-count, facade, continuation and semantic-invalid poll ceilings to attribute the defect. Implement only an internal repair proved by both compiler and product evidence; no public ISA/ABI, product-default, push, merge or cross-platform selection is authorized. Control: [`PERF3-05-R3-WORKLIST.md`](PERF3-05-R3-WORKLIST.md). |
