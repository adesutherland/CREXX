...
edit. F1a-F1d are now complete: the five RXAS operations execute on both
...
slots `466..471` reserved. The F1d paired verdict has no confirmed regression;
the separately accepted computed-goto code-layout cost remains recorded for
final hot-loop hardening. F1e isolated-process tasks are next. Evidence:
...
Design:
[`Gate F public concurrency surface`](PERF3-13-GATE-F-DESIGN.md). User model:
[`Gate F concurrency user guide`](PERF3-13-GATE-F-USER-GUIDE.md). Execution:
[`Gate F implementation plan`](PERF3-13-GATE-F-IMPLEMENTATION-PLAN.md).
...
| PERF3-05-R4 | P2 | RXAS/RXVM fusion registry and quickening contract | queued documentation/design item — no implementation selected | Inventory every RXAS static fusion beside every VM load-time private execution-image fusion, including the canonical sequence and operand proof, static/runtime eligibility, guard and fallback, debugger/TRACE/source/signal/profiling identity, and measured benefit. Produce an explicit gap report so transformations RXAS cannot safely prove are visible rather than hidden in private VM code. Decide separately whether any private fusion should become a normal public instruction; that is an RXAS/RXBIN architecture and compatibility gate, not part of handler placement. Preserve canonical RXBIN and immutable per-load rewriting unless a later approved design proves a need for adaptive runtime quickening or dequickening. |
| PERF3-05-R5 | P1 | Handler-placement percentage, never-inline and platform panel | R5b Apple closeout complete — profile-20 default; Intel Linux next | Adrian selected common profile-20 as the provisional default and accepted the known GCC threaded Bounce guard. Formal Clang improves +3.857%/+3.152% for `rxtvm`/`rxbvm`; GCC improves +3.175%/+9.646% overall but Bounce loses 10.072% on threaded dispatch. On this host Clang profile-20 is directionally faster than GCC in all 14 workload/engine cells, with unpaired all-seven point estimates of +23.8%/+41.5%. Fresh no-option and explicit profile-20 Release binaries are byte-identical for both engines; default Release and Debug each pass 2,002/2,002 tests and focused profile/documentation QA passes 6/6. Effective placement remains visible as `inline`/`outline`/`mixed`. Intel Linux validation and sanitizer work are next; Windows remains a later lane. During Release 1 finalisation, regenerate the exact panel from a wider current portfolio and retain membership changes as code/workloads evolve. Control: [`PERF3-05-R5-WORKLIST.md`](PERF3-05-R5-WORKLIST.md); evidence: [`R5 panel`](evidence/2026-08-10-perf3-05-r5-handler-percentage-panel/), [`R5a placement`](evidence/2026-08-10-perf3-05-r5a-handler-placement-profiling/), [`R5b closeout`](evidence/2026-08-10-perf3-05-r5b-profile20-default-closeout/). |
...
| PERF3-13 / CAP-06 | P0 | RXVM allocator, value-shape and worker-communication foundation | Gate E E6 C0 accepted and Mac QA closed; Gate F F0-S through F1d complete; F1e next | Gate C selected the 176-byte L32SDH value; Gate D industrialised worker slabs; EF-0 and E1/E2 established explicit worker memory, lifecycle and active-state ownership. E3 retains the legacy RXPA ABI while adding process-reentrant opt-in, branch-free load-selected invocation, optional per-VM sessions and the ODBC ownership example. E4 retains independent-load control and implements runtime-owned sealed immutable generations with worker-owned overlays. E5 integrates POSIX and Windows native carriers plus the targetable-only sparse fallback, correlated mailbox, copied logical request image, typed completion and deterministic terminal lifecycle; closure `9f5bb579a` is integrated by develop merge `795e58edb`, and its published matrix is green. E6 selects strict C0 owner-token enforcement and permanent private 1/2/4/8-worker compute/churn qualification. Automatic quiescent reclaim C1 and remote-free C2 are rejected and removed; the cleaned form passes focused Debug/Apple-ASan/Release 49/49, complete Debug and Release builds, and full Debug 2,080/2,080. The recorded Gate F ownership surface keeps globals execution-local, represents durable mutable state as single-owner services/actors, transfers only `ChannelValue`/immutable binary, requires structured scopes and terminal completion, and binds the Level B classes to mandatory transport-neutral RXAS channel instructions implemented by RXVM. Adrian approved the typed task-expression and `DO PARALLEL` Rexx surface, complete Level B class mapping, RXAS-only systems bridge, reusable byte-endpoint/child-process migration and concurrent HTTP consumer on 2026-08-14. F0-S fixes the exact machine/class/provider contract, opcodes `650..654`, RXCV encoding, diagnostics and conformance vectors before the first code edit. F1a-F1d now deliver those five opcodes, the complete both-VM local provider, full RXCV/lifecycle, private provider conformance, the executable Level B surface, reusable byte endpoints, structured child processes and ADDRESS migration; retired process/redirect slots remain reserved. The F1d verdict has no confirmed regression, while the earlier computed-goto code-layout cost remains recorded for F3 final hardening. F1e isolated-process tasks are next; each production slice retains its first Release verdict. Control: [`PERF3-13-WORKLIST.md`](PERF3-13-WORKLIST.md); user guide: [`PERF3-13-GATE-F-USER-GUIDE.md`](PERF3-13-GATE-F-USER-GUIDE.md); AI specification: [`PERF3-13-GATE-F-AI-SPEC.md`](PERF3-13-GATE-F-AI-SPEC.md); implementation plan: [`PERF3-13-GATE-F-IMPLEMENTATION-PLAN.md`](PERF3-13-GATE-F-IMPLEMENTATION-PLAN.md); F1a/F1b evidence: [`first Release verdict and closeout`](evidence/2026-08-14-perf3-13-gate-f-f1ab-first-release-verdict/); F1c evidence: [`full local surface verdict and closeout`](evidence/2026-08-14-perf3-13-gate-f-f1c-first-release-verdict/); F1d evidence: [`byte endpoint and child process verdict and closeout`](evidence/2026-08-15-perf3-13-gate-f-f1d-first-release-verdict/); E6 evidence: [`C0 selection and closeout`](evidence/2026-08-13-perf3-13-gate-e-e6-first-release-verdict/); Gate F design: [`PERF3-13-GATE-F-DESIGN.md`](PERF3-13-GATE-F-DESIGN.md); E5 evidence: [`industrial closeout`](evidence/2026-08-13-perf3-13-gate-e-e5-industrial-closeout/), [`macOS`](evidence/2026-08-12-perf3-13-gate-e-e5-macos-doorbell-poc/), [`Linux GCC`](evidence/2026-08-12-perf3-13-gate-e-e5-linux-doorbell-poc/) and [`Linux Clang`](evidence/2026-08-12-perf3-13-gate-e-e5-linux-clang-doorbell-poc/); E4b evidence: [`sealed-generation verdict and closeout`](evidence/2026-08-11-perf3-13-gate-e-e4b-first-release-verdict/). |
