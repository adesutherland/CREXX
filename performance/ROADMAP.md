...
edit. F1a-F1c are now complete: the five RXAS operations execute on both
concrete VMs with full canonical RXCV, typed register images,
generation-checked capabilities, private provider conformance,
deadline/scope/lifecycle control and explicit Level B classes. The confirmed
computed-goto code-layout cost is recorded for final hot-loop hardening after
the instruction surface stabilizes. F1d reusable byte endpoints and
child-process/redirect integration are next. Evidence:
