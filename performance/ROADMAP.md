...
| LEVEL-L-INLINE-01 | P1 experimental | Cross-file inline storage proof for Level L workloads | staged on local hotfix; focused proof passed; broader QA pending | I7 adds fail-closed structural receiver-storage proofs for exact non-reference concrete objects, direct indexed non-reference arrays, bounded nonescaping child-binary access, and exact residual member dependencies while preserving I6 for existing shapes. The follow-up makes imported methods use the same receiver gate as local methods: read-only receiver expressions are evaluated once, while statement-owned mutation is limited to direct receivers or captured variable-like locators with evaluate-once copyback. References, escaping/whole-value replacement, interface/dynamic dispatch and receiver-producing mutating expressions remain ordinary calls. The baseline and parity follow-up pass the focused source/binary, optimized/no-opt, RXDAS and dual-VM regression; the parity assertions cover a factory-produced read-only receiver plus an indexed receiver whose selector is evaluated once before mutating copyback. The 2026-08-26 profiling-off Release baseline verdict retained 2 warmups plus 12 balanced pairs per VM with all checksums passing: paired median elapsed fell 65.42% on `rxtvm` and 64.47% on `rxbvm`, with 12/12 favourable pairs in both cells. Evidence: [`2026-08-26-level-l-inline-storage-first-release-verdict`](evidence/2026-08-26-level-l-inline-storage-first-release-verdict/). Publication remains gated on the hotfix QA handoff. |
...
| LEVEL-B-CALL-POSTFIX | P2 language/parser | Computed-receiver standalone `CALL` surface | staged on local hotfix; focused proof passed; broader QA pending | The dedicated Level B `CALL` grammar now has a zero-conflict member-postfix target which requires a final method suffix. Indexed, parenthesized, factory/function-produced and chained receivers are admitted without treating arbitrary expressions as statements or changing classic `CALL name arg,...`. Focused optimized/no-opt runtime coverage proves the surface forms; the indexed receiver is evaluated once, mutation is copied back, and optimized RXAS contains no residual target method call. Broader regression and Level C remapping qualification remain hotfix QA work before publication. |
| RXC-CALL-REG-01 | P1 compiler defect | Statement-call and temporary member-receiver register lifetime | staged on local hotfix; focused optimized/no-opt runtime proof passed; broader QA pending | Thousands of mutually exclusive generated call sites exposed three allocator lifetime omissions: discarded standalone-call results were retained, temporary member receivers outside the reserved call window were retained, and a statement call's node register was returned too late to be reused by deferred cleanup. The repair recycles those temporaries without changing call-window ownership. The permanent 32-call regression compiles, assembles and runs in optimized and no-opt modes, and caps the dispatch procedure at 7 and 39 locals respectively; the retained pre-fix no-opt image required 101. The Unicode 3,440-state diagnostic remains evidence of scale, not a normalization-performance claim: its scratch `normalize()` frame fell from 6,455 to 361 locals without materially improving wall time. Publication remains gated on hotfix QA. |
