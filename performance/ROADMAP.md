...
edit. F1a and the minimum F1b type `1` local-provider slice are now complete;
Adrian accepted the first Release verdict and Mac closeout, and F1c is next.
The five RXAS operations execute on both concrete VMs with canonical minimum
RXCV, generation-checked capabilities, bounded admission/cancellation,
completion ordering and deterministic teardown. The accepted computed-goto
code-layout cost is deferred to final hot-loop hardening after the instruction
surface stabilizes. Evidence:
