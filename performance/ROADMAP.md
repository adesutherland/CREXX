...
| PERF3-05-R2 | P1 | Profile-selected RXAS/RXBIN instruction-handler inline/call framework | in progress — approved through the Apple 30% inline checkpoint | Adrian approved autonomous comparative processing on 2026-08-09 for both concrete engines, `rxtvm` and `rxbvm`. Express every interpreter instruction handler once behind an internal macro/definition boundary that can emit a hot handler directly inside the dispatch owner or compile a cold handler as a normal callable function. Group handler definitions into coherent internal files for maintenance while preserving one semantic source. Select the inline set from dynamic instruction profiles plus native I-cache/branch/call evidence, not intuition; this may control `run()` size and limit accidental compiler macro-propagation or mega-function flattening. The PoC must define the called-handler state/continuation ABI and preserve PC movement, register/reference ownership, signals, interrupts, TRACE, profiling, native/plugin calls and exactly identical terminal behavior under portable switch and direct-threaded dispatch. Compare current, all-inline, all-outlined and cumulative profile-selected panels through at most 176 of 588 non-reserved public handlers inline. Retain text/resident-hot size, call/branch cost, build time/RSS, artifact/RSS and representative end-to-end timing for both concrete VMs. This extends PERF3-05-R1; it authorizes no public RXAS/RXBIN or plugin ABI change and stops before production selection or cross-platform/default-VM closure. Control: [`PERF3-05-R2-WORKLIST.md`](PERF3-05-R2-WORKLIST.md). |
