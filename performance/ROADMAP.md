...
| PERF3-05-R5 | P1 | Handler-placement percentage, never-inline and platform panel | R5a placement diagnosis complete — code-shape cause, no default selected | All 651 handlers now have one auditable heat/eligibility tier; 56 host-bound operations plus reserved/sentinel handlers stay callable in the 531/589 (90.15%) `max-eligible` panel. Formal Clang 20% is +3.857%/+3.152% for `rxtvm`/`rxbvm`; formal GCC is +3.175%/+9.646% but `rxtvm` Bounce regresses 10.072%. Profiles now report effective `inline`/`outline`/`mixed` placement without hiding private fusion. GCC Bounce executes 887,867,426 instructions: only 424,204 (0.047778%) are outlined at 20%, and 424,200 are `CALL1`. At 30% only two one-off `SAY`s remain outlined, exactly the same dynamic placement as max-eligible, yet threaded Bounce is -8.691% versus -4.693%. The defect is therefore compiler owner/code shape from unexecuted handlers, not outlined call frequency. No tier/default change; Linux/Windows and sanitizer expansion remain gated. Refresh the exact portfolio mix before release. Control: [`PERF3-05-R5-WORKLIST.md`](PERF3-05-R5-WORKLIST.md); evidence: [`R5 panel`](evidence/2026-08-10-perf3-05-r5-handler-percentage-panel/), [`R5a placement`](evidence/2026-08-10-perf3-05-r5a-handler-placement-profiling/). |
