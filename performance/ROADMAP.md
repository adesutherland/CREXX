...
| LEVEL-L-CALL-01 | P2 language/parser | Computed-receiver standalone `CALL` surface | queued language-design review | Level B's dedicated `CALL` production accepts a simple symbol/stem member chain but does not reuse the general postfix-expression grammar, so `call object.method()` is valid while `call objects[index].method()` is rejected even though the same member call is valid as an expression. Decide whether to admit computed member calls after `CALL` or add an explicit ignored-result expression statement; preserve evaluate-once receiver semantics, signals, diagnostics and Level C remapping. This is independent of imported-receiver inline parity. |
