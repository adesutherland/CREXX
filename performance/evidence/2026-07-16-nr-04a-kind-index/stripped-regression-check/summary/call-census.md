# NR-05 call-path census dashboard

Dynamic schema-5 observations aggregated across 4 exact images. Counts reflect each manifest cell's disclosed bounded argv; they are census evidence, not product timing or a performance-win claim.

## Calls by runtime path

| Path | Calls |
|---|---:|
| direct bytecode | 10281800 |
| direct native | 0 |
| dynamic bytecode | 0 |
| dynamic native | 0 |
| external/root | 4 |
| signal bytecode | 0 |
| signal native | 0 |

## Actual arity

| Arity | Calls |
|---:|---:|
| 1 | 7649604 |
| 3 | 568600 |
| 2 | 21800 |
| 4 | 561800 |
| 5 | 10000 |
| 13 | 1310000 |
| 14 | 160000 |

## Callable and frame disposition

| Category | Count |
|---|---:|
| procedure | 1490004 |
| method | 8785600 |
| factory | 6200 |
| native | 0 |
| unknown | 0 |
| fresh frames | 116 |
| reused frames | 10281688 |
| native no-child | 0 |
| failed/no frame | 0 |

## Call-window mechanics

| Mechanic | Count | Per instruction call |
|---|---:|---:|
| setup swaps | 30768800 | 2.992550 |
| normal restoration swaps | 30768800 | 2.992550 |
| defensive argument copies | 200 | 0.000019 |
| unclassified swaps | 0 | n/a |
| unclassified copies | 8355604 | n/a |
| degraded attributions | 0 | n/a |

## Return placement

| Placement | Count |
|---|---:|
| void | 5600 |
| RET_REG local move | 631600 |
| RET_REG non-local copy | 8062600 |
| ignored value | 0 |
| immediate | 1582000 |
| terminal/external | 4 |

## Dynamic selection and signal unwind

| Counter | Count |
|---|---:|
| method selections | 0 |
| method selection failures | 0 |
| factory selections | 0 |
| factory selection failures | 0 |
| signal unwind events | 0 |
| bytecode frames discarded | 0 |
| bytecode windows restored | 0 |
| bytecode slots restored | 0 |
| native windows restored | 0 |
| native slots restored | 0 |
| restoration failures | 0 |

## NR-06 and NR-12 handoff

- NR-06 call-window work should use the path and exact-arity distributions above, together with 30768800 setup and 30768800 restoration swaps (5.985100 combined per instruction call across this portfolio run).
- Frame recycling supplied 10281688 of 10281804 observed bytecode activations; preserve the per-workload rows before generalising from the aggregate.
- NR-12 should distinguish 200 attributable argument-copy instructions from the unclassified copy population, and should treat 8062600 measured non-local RET_REG copies separately from 631600 local moves.
- Dynamic selection occurred 0 times and signal unwind occurred 0 times. Zero values mean unobserved in these bounded portfolio cells, not unreachable VM paths; focused semantic fixtures cover the cold paths.
- These aggregate conclusions are portfolio-wide for the exact 4-image set. Workload-specific conclusions must be drawn from call-census.csv and the retained raw profile for that entry.
