...
def paired_description(values):
    ...
    if len(values) > 1:
        ...
        # Two-sided Student t, 95%, df=11. The campaign is fixed at n=12.
        half_width = 2.201 * standard_error
    else:
        stdev = standard_error = half_width = 0.0
    result.update(
        stdev=stdev,
        ...
        summary_rows.append(
            {
                ...
                "native_cps_favorable_count": sum(
                    float(row["native_cps_delta_percent"]) > 0 for row in rows
                ),
                ...
                "process_real_favorable_count": sum(
                    float(row["process_real_delta_percent"]) < 0 for row in rows
                ),
...
write_csv(
    output_dir / "position-summary.csv",
    list(position_rows[0]),
    position_rows,
)

evidence_dir = output_dir.parent.parent
with (evidence_dir / "2026-07-17-nr-09-numctx-first-release-verdict" / "candidate-summary.csv").open(newline="") as source:
    retained_baseline = {row["vm"]: row for row in csv.DictReader(source)}
with (output_dir.parent / "candidate-summary.csv").open(newline="") as source:
    retained_candidate = {row["vm"]: row for row in csv.DictReader(source)}
with (output_dir.parent / "comparison.csv").open(newline="") as source:
    retained_comparison = {row["vm"]: row for row in csv.DictReader(source)}

cell_lookup = {(row["vm_mode"], row["cell"]): row for row in cell_rows}
paired_lookup = {
    (row["vm_mode"], row["comparison"]): row for row in summary_rows
}
historical_rows = []
for vm_mode in ("rxvm", "rxbvm"):
    old_a = retained_baseline[vm_mode]
    old_c = retained_candidate[vm_mode]
    old_comparison = retained_comparison[vm_mode]
    new_a = cell_lookup[(vm_mode, "A")]
    new_c = cell_lookup[(vm_mode, "C")]
    paired = paired_lookup[(vm_mode, "overall_C_vs_A")]

    old_a_cps = float(old_a["native_cps_median"])
    old_c_cps = float(old_c["native_cps_median"])
    new_a_cps = float(new_a["native_cps_median"])
    new_c_cps = float(new_c["native_cps_median"])
    old_a_real = float(old_a["process_real_median_s"])
    old_c_real = float(old_c["process_real_median_s"])
    new_a_real = float(new_a["process_real_median_s"])
    new_c_real = float(new_c["process_real_median_s"])

    historical_rows.append(
        {
            "vm_mode": vm_mode,
            "retained_baseline_cps_median": f"{old_a_cps:.0f}",
            "rebuilt_baseline_cell_A_cps_median": f"{new_a_cps:.3f}",
            "baseline_session_drift_percent": f"{((new_a_cps / old_a_cps) - 1.0) * 100.0:+.6f}",
            "retained_candidate_cps_median": f"{old_c_cps:.0f}",
            "rerun_candidate_cell_C_cps_median": f"{new_c_cps:.3f}",
            "candidate_session_drift_percent": f"{((new_c_cps / old_c_cps) - 1.0) * 100.0:+.6f}",
            "retained_cross_session_cps_delta_percent": old_comparison["cps_delta_percent"],
            "rerun_cell_median_cps_delta_percent": f"{((new_c_cps / new_a_cps) - 1.0) * 100.0:+.6f}",
            "rerun_paired_median_cps_delta_percent": paired["native_cps_delta_median_percent"],
            "retained_baseline_real_median_s": f"{old_a_real:.2f}",
            "rebuilt_baseline_cell_A_real_median_s": f"{new_a_real:.3f}",
            "baseline_real_session_drift_percent": f"{((new_a_real / old_a_real) - 1.0) * 100.0:+.6f}",
            "retained_candidate_real_median_s": f"{old_c_real:.2f}",
            "rerun_candidate_cell_C_real_median_s": f"{new_c_real:.3f}",
            "candidate_real_session_drift_percent": f"{((new_c_real / old_c_real) - 1.0) * 100.0:+.6f}",
            "retained_cross_session_real_delta_percent": old_comparison["real_delta_percent"],
            "rerun_cell_median_real_delta_percent": f"{((new_c_real / new_a_real) - 1.0) * 100.0:+.6f}",
            "rerun_paired_median_real_delta_percent": paired["process_real_delta_median_percent"],
        }
    )

write_csv(
    output_dir / "historical-drift.csv",
