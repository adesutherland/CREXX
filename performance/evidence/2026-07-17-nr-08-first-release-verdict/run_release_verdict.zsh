...
baseline_dir="$repo_root/performance/evidence/2026-07-17-numeric-01-first-release-verdict/accepted-c"
...
if [[ ! -f "$baseline_dir/samples.csv" || ! -f "$baseline_dir/summary.csv" ]]; then
  print -u2 'accepted NUMERIC-01 baseline evidence is missing'
  exit 1
fi
...
run_sample() {
  ...
  if [[ -z "$native_cps" || -z "$process_real_s" || "$effective_count" != 100 || "$pass" != 1 ]]; then
    print -u2 "invalid NR-08 candidate sample: $stem"
    exit 1
  fi
...
for vm_name in rxvm rxbvm; do
  cps_values=$(mktemp /tmp/crexx-nr08-cps.XXXXXX)
  wall_values=$(mktemp /tmp/crexx-nr08-wall.XXXXXX)
...
for vm_name in rxvm rxbvm; do
  baseline_row=$(awk -F, -v vm="$vm_name" '$1 == vm {print; exit}' "$baseline_dir/summary.csv")
...

{
  ...
  shasum -a 256 "$baseline_dir/samples.csv"
  shasum -a 256 "$baseline_dir/summary.csv"
...
print
print 'Retained-baseline comparison:'
