...
"set GF_CONTROL_DIR to the retained committed F1g-C Release product directory"}
gf_accepted_image=${GF_ACCEPTED_IMAGE:?
"set GF_ACCEPTED_IMAGE to the sealed F1g-D-compatible benchmark image"}
gf_candidate_image=${GF_CANDIDATE_IMAGE:-"$gf_build_dir/interpreter/test_gate_f_channel_benchmark.rxbin"}
gf_output_dir=${1:?
"usage: capture-task-paired.zsh OUTPUT_DIR"}
...
gf_run_channel() {
    ...
    if [[ "$gf_variant" == accepted ]]; then
        ...
        gf_image="$gf_accepted_image"
    else
        gf_product="$gf_build_dir/bin/$gf_product_name"
        gf_image="$gf_candidate_image"
    fi
